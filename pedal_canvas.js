...
(function () {
  ...
  function switchSquare(d, cx, cy, hs, on, dark, col) { const c = d.ctx;
    if (dark) {
    ...
    // col (optional [r,g,b]) overrides the default red cap so custom gear can
    // recolour a switch; off state uses a dimmed version of the same hue.
    const onC  = col ? rgb(col[0], col[1], col[2]) : rgb(208,40,36);
    const offC = col ? rgb(col[0]*0.38, col[1]*0.38, col[2]*0.38) : rgb(78,22,20);
    rr(c, cx-hs, cy-hs, hs*2, hs*2, 3); c.fillStyle = on ? onC : offC; c.fill();
  ...
  function hSlider(d, x0, x1, y, val, col) { const c = d.ctx, s = d.s;
    ...
    // col (optional [r,g,b]) tints the draggable cap for custom gear.
    const capHi = col ? rgb(clamp(col[0]+30,0,255),clamp(col[1]+30,0,255),clamp(col[2]+30,0,255)) : rgb(48,48,52);
    const capLo = col ? rgb(col[0]*0.45,col[1]*0.45,col[2]*0.45) : rgb(18,18,20);
    const g = c.createLinearGradient(cx-cw/2, y-ch/2, cx+cw/2, y+ch/2); g.addColorStop(0, capHi); g.addColorStop(1, capLo);
  ...
  function knob(d, cx, cy, R, val, style, capR, capG, capB, tickCol, ptrCol) {
    ...
    if (style==='irt') {
      // Laney IRT: perilla negra glossy con falda suave y FRANJA blanca ancha.
      c.beginPath(); c.arc(cx,cy,R*1.06,0,7); c.fillStyle=rgb(8,8,9); c.fill();
      const bg=c.createRadialGradient(cx-R*0.35,cy-R*0.45,R*0.08,cx,cy,R);
      bg.addColorStop(0,rgb(96,98,104)); bg.addColorStop(0.35,rgb(44,45,49)); bg.addColorStop(1,rgb(16,16,18));
      c.beginPath(); c.arc(cx,cy,R,0,7); c.fillStyle=bg; c.fill();
      c.strokeStyle=rgb(4,4,5); c.lineWidth=1.2*s; c.stroke();
      // falda: estrias suaves
      c.strokeStyle='rgba(255,255,255,0.05)'; c.lineWidth=1*s;
      for(let i=0;i<24;i++){ const t=i/24*Math.PI*2; c.beginPath();
        c.moveTo(cx+R*0.84*Math.cos(t),cy+R*0.84*Math.sin(t)); c.lineTo(cx+R*0.98*Math.cos(t),cy+R*0.98*Math.sin(t)); c.stroke(); }
      // cara superior glossy
      const fg=c.createRadialGradient(cx-R*0.22,cy-R*0.3,R*0.04,cx,cy,R*0.68);
      fg.addColorStop(0,rgb(70,72,78)); fg.addColorStop(0.5,rgb(28,28,32)); fg.addColorStop(1,rgb(12,12,14));
      c.beginPath(); c.arc(cx,cy,R*0.66,0,7); c.fillStyle=fg; c.fill();
      c.strokeStyle='rgba(255,255,255,0.10)'; c.lineWidth=0.8*s; c.stroke();
      // reflejo superior
      c.save(); c.globalAlpha=0.18; c.beginPath();
      c.ellipse(cx-R*0.10,cy-R*0.30,R*0.42,R*0.20,-0.5,0,7); c.fillStyle=rgb(255,255,255); c.fill(); c.restore();
      // FRANJA blanca del puntero (ancha, de centro a borde como el IRT)
      c.save(); c.translate(cx,cy); c.rotate(a);
      rr(c,R*0.10,-2.4*s,R*0.92,4.8*s,2*s); c.fillStyle=rgb(240,242,246); c.fill();
      c.strokeStyle='rgba(0,0,0,0.35)'; c.lineWidth=0.7*s; c.stroke(); c.restore();
      return;
    }
    if (style==='vh4') {
      // Diezel VH4: perilla negra con LOBULOS de agarre (gira con el valor),
      // cara conica glossy y tornillo hex plateado al centro.
      const lobes=8;
      c.beginPath();
      for(let i=0;i<=lobes*10;i++){ const t=i/(lobes*10)*Math.PI*2;
        const rr2=R*(1.0-0.13*Math.pow((Math.cos(lobes*(t-a))+1)*0.5,1.6));
        const x=cx+rr2*Math.cos(t), y=cy+rr2*Math.sin(t);
        i?c.lineTo(x,y):c.moveTo(x,y); }
      c.closePath();
      const bg=c.createRadialGradient(cx-R*0.35,cy-R*0.4,R*0.1,cx,cy,R*1.05);
      bg.addColorStop(0,rgb(74,76,82)); bg.addColorStop(0.5,rgb(30,30,34)); bg.addColorStop(1,rgb(10,10,12));
      c.fillStyle=bg; c.fill(); c.strokeStyle=rgb(4,4,6); c.lineWidth=1.2*s; c.stroke();
      // cara conica
      const fg=c.createRadialGradient(cx-R*0.25,cy-R*0.3,R*0.05,cx,cy,R*0.62);
      fg.addColorStop(0,rgb(58,60,66)); fg.addColorStop(1,rgb(16,16,18));
      c.beginPath(); c.arc(cx,cy,R*0.60,0,7); c.fillStyle=fg; c.fill();
      c.strokeStyle='rgba(255,255,255,0.08)'; c.lineWidth=1*s; c.stroke();
      // indicador blanco sobre la cara
      c.beginPath(); c.moveTo(cx+R*0.28*Math.cos(a),cy+R*0.28*Math.sin(a)); c.lineTo(cx+R*0.86*Math.cos(a),cy+R*0.86*Math.sin(a));
      c.strokeStyle=rgb(235,237,241); c.lineWidth=2.6*s; c.lineCap='round'; c.stroke(); c.lineCap='butt';
      // tornillo hex central plateado
      const sg=c.createRadialGradient(cx-R*0.06,cy-R*0.08,R*0.02,cx,cy,R*0.20);
      sg.addColorStop(0,rgb(225,227,232)); sg.addColorStop(1,rgb(128,130,136));
      c.beginPath(); c.arc(cx,cy,R*0.18,0,7); c.fillStyle=sg; c.fill();
      c.strokeStyle=rgb(60,62,66); c.lineWidth=0.8*s; c.stroke();
      c.beginPath();
      for(let i=0;i<6;i++){ const t=i/6*Math.PI*2+0.3; const x=cx+R*0.10*Math.cos(t), y=cy+R*0.10*Math.sin(t);
        i?c.lineTo(x,y):c.moveTo(x,y); }
      c.closePath(); c.fillStyle=rgb(52,54,58); c.fill();
      return;
    }
    if (style==='davies') {
    ...
    if (style==='skirt') {
      // Fender cream SKIRTED (Super-Sonic/blackface): falda estriada con los
      // numeros 1-10 impresos + tapa domo crema que gira con linea indicadora.
      c.save(); c.globalAlpha=0.28; c.beginPath();
      c.ellipse(cx+1.2*s,cy+2*s,R*1.30,R*1.24,0,0,7); c.fillStyle=rgb(0,0,0); c.fill(); c.restore();
      const sk=c.createRadialGradient(cx-R*0.35,cy-R*0.42,R*0.1,cx,cy,R*1.32);
      sk.addColorStop(0,rgb(238,231,207)); sk.addColorStop(0.7,rgb(218,209,180)); sk.addColorStop(1,rgb(158,148,120));
      c.beginPath(); c.arc(cx,cy,R*1.28,0,7); c.fillStyle=sk; c.fill();
      c.strokeStyle=rgb(110,102,78); c.lineWidth=1*s; c.stroke();
      c.strokeStyle='rgba(120,110,82,0.5)'; c.lineWidth=0.8*s;
      for(let i=0;i<24;i++){ const t=i/24*Math.PI*2; c.beginPath();
        c.arc(cx+R*1.17*Math.cos(t),cy+R*1.17*Math.sin(t),R*0.115,t-2.2,t+2.2); c.stroke(); }
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      c.font=`700 ${Math.max(6.5*s,R*0.30)}px ${FONTS.barlow}, sans-serif`; c.fillStyle=rgb(72,62,44);
      for(let n=1;n<=10;n++){ const aa=ang((n-1)/9);
        c.fillText(String(n), cx+R*0.99*Math.cos(aa), cy+R*0.99*Math.sin(aa)); }
      c.restore();
      const cg2=c.createRadialGradient(cx-R*0.28,cy-R*0.34,R*0.08,cx,cy,R*0.80);
      cg2.addColorStop(0,rgb(250,244,224)); cg2.addColorStop(0.6,rgb(232,223,195)); cg2.addColorStop(1,rgb(188,178,148));
      c.beginPath(); c.arc(cx,cy,R*0.74,0,7); c.fillStyle=cg2; c.fill();
      c.strokeStyle='rgba(140,130,100,0.7)'; c.lineWidth=0.9*s; c.stroke();
      c.strokeStyle='rgba(120,110,82,0.40)'; c.lineWidth=0.6*s;
      for(let i=0;i<40;i++){ const t=i/40*Math.PI*2; c.beginPath();
        c.moveTo(cx+R*0.64*Math.cos(t),cy+R*0.64*Math.sin(t)); c.lineTo(cx+R*0.73*Math.cos(t),cy+R*0.73*Math.sin(t)); c.stroke(); }
      c.beginPath(); c.moveTo(cx+R*0.10*Math.cos(a),cy+R*0.10*Math.sin(a));
      c.lineTo(cx+R*0.70*Math.cos(a),cy+R*0.70*Math.sin(a));
      c.lineCap='round'; c.strokeStyle=rgb(52,44,32); c.lineWidth=2.6*s; c.stroke(); c.lineCap='butt';
      c.beginPath(); c.arc(cx-R*0.24,cy-R*0.28,R*0.13,0,7); c.fillStyle='rgba(255,253,246,0.45)'; c.fill();
      return;
    }
    if (style==='plexigold') {
      // Marshall: tapa DORADA torneada dominante + falda negra acanalada
      // FINA con ranura indicadora negra (como el knob gold-top real).
      ...
      // falda negra fina (gira con el valor)
      ...
      c.lineWidth=1.8*s;
      for(let i=0;i<28;i++){ const t=i/28*Math.PI*2+a;
        c.strokeStyle=(i%2)?'rgba(0,0,0,0.55)':'rgba(255,255,255,0.08)';
        c.beginPath();
      ...
      c.save(); c.beginPath(); c.arc(cx,cy,R*0.84,0,7); c.clip();
      c.globalAlpha=0.30; c.beginPath();
      c.moveTo(cx-R*0.7,cy+R*0.15); c.lineTo(cx-R*0.05,cy-R*0.85); c.lineTo(cx+R*0.25,cy-R*0.85); c.lineTo(cx-R*0.4,cy+R*0.15); c.closePath();
      ...
      c.lineTo(cx+R*0.80*Math.cos(a),cy+R*0.80*Math.sin(a));
      ...
      c.lineTo(cx+R*0.80*Math.cos(a)+1*s,cy+R*0.80*Math.sin(a)+1*s); c.stroke(); c.lineCap='butt';
      return;
    }
    if (style==='jtmsilver') {
      // JTM45 MkII: perilla skirtless de ALUMINIO hilado (plateada) con linea
      // indicadora oscura, como las del panel dorado de la referencia.
      c.save(); c.globalAlpha=0.30; c.beginPath();
      c.ellipse(cx+1.2*s,cy+2*s,R*1.06,R*1.00,0,0,7); c.fillStyle=rgb(0,0,0); c.fill(); c.restore();
      // cuerpo de aluminio (borde levemente conico mas oscuro)
      const bd=c.createRadialGradient(cx-R*0.32,cy-R*0.38,R*0.08,cx,cy,R*1.06);
      bd.addColorStop(0,rgb(250,251,253)); bd.addColorStop(.55,rgb(202,205,210)); bd.addColorStop(1,rgb(126,129,135));
      c.beginPath(); c.arc(cx,cy,R,0,7); c.fillStyle=bd; c.fill();
      c.strokeStyle=rgb(84,87,92); c.lineWidth=1*s; c.stroke();
      // moleteado fino del borde (gira con el valor)
      c.lineWidth=1.2*s;
      for(let i=0;i<40;i++){ const t=i/40*Math.PI*2+a;
        c.strokeStyle=(i%2)?'rgba(0,0,0,0.20)':'rgba(255,255,255,0.35)';
        c.beginPath();
        c.moveTo(cx+R*0.88*Math.cos(t),cy+R*0.88*Math.sin(t));
        c.lineTo(cx+R*0.99*Math.cos(t),cy+R*0.99*Math.sin(t)); c.stroke(); }
      // tapa hilada con anillos concentricos del torneado
      const tg3=c.createRadialGradient(cx-R*0.26,cy-R*0.30,R*0.05,cx,cy,R*0.84);
      tg3.addColorStop(0,rgb(246,248,250)); tg3.addColorStop(.55,rgb(212,215,220)); tg3.addColorStop(1,rgb(158,161,167));
      c.beginPath(); c.arc(cx,cy,R*0.84,0,7); c.fillStyle=tg3; c.fill();
      c.strokeStyle=rgb(120,123,128); c.lineWidth=0.8*s; c.stroke();
      for(let rr2=R*0.14; rr2<R*0.80; rr2+=R*0.085){
        c.beginPath(); c.arc(cx,cy,rr2,0,7);
        c.strokeStyle='rgba(96,99,104,0.22)'; c.lineWidth=0.7*s; c.stroke(); }
      // destello diagonal del hilado
      c.save(); c.beginPath(); c.arc(cx,cy,R*0.82,0,7); c.clip();
      c.globalAlpha=0.32; c.beginPath();
      c.moveTo(cx-R*0.7,cy+R*0.2); c.lineTo(cx-R*0.05,cy-R*0.85); c.lineTo(cx+R*0.25,cy-R*0.85); c.lineTo(cx-R*0.4,cy+R*0.2); c.closePath();
      c.fillStyle=rgb(255,255,255); c.fill(); c.restore();
      // linea indicadora oscura
      c.beginPath(); c.moveTo(cx+R*0.14*Math.cos(a),cy+R*0.14*Math.sin(a));
      c.lineTo(cx+R*0.80*Math.cos(a),cy+R*0.80*Math.sin(a));
      c.lineCap='round'; c.strokeStyle=rgb(44,46,50); c.lineWidth=2.4*s; c.stroke(); c.lineCap='butt';
      return;
    }
    if (style==='jcsilver') {
  ...
  // Real BF-2 controls: Manual0 Depth1 Rate2 Res3.
  ...
  // Real controls: Level0 Tone1 Release2 Threshold3.
  P.lm2 = chiefSpec(300,480,[64,184,228],
    [{id:0,cx:.205,lbl:'LEVEL'},{id:1,cx:.40,lbl:'TONE'},{id:2,cx:.595,lbl:'RELEASE',lblPx:7.5},{id:3,cx:.79,lbl:'THRESH',lblPx:7.5}],
    'Limiter',null,'LM-2');
  ...
  // Swole — Lazy Sprocket / SG-1-style slow gear: orange box, black top
  // banner, dB gain-reduction meter, two scaled knobs, distressed wordmark + LED
  // + stomp. Parody (Beta Fist Factory). Real controls: Sensitivity0 Attack1.
  P.betafist = { w:460,h:330, knobs:[
    ...
    draw(d){ const {ctx:c,W,H,s}=d; const m=8*s, ink=rgb(24,20,16), wt=rgb(240,240,242);
      ...
      rr(c,m,m,W-2*m,H-2*m,12*s); c.fillStyle=bg; c.fill();
      rr(c,m,m,W-2*m,H-2*m,12*s); c.strokeStyle='rgba(0,0,0,0.4)'; c.lineWidth=2*s; c.stroke();
      // black top banner + slow-gear model text
      c.save(); rr(c,m,m,W-2*m,H-2*m,12*s); c.clip();
      ...
      c.restore();
      textSpaced(d,.40*W,.130*H,F.crete,20,wt,'Slow Gear',0.2);
      ...
      textSpaced(d,.16*W,.705*H,F.barlow,8,ink,'SENSITIVITY',0.1);
      textSpaced(d,.80*W,.475*H,F.barlow,9,ink,'ATTACK',0.3);
  ...
  // Dyna Compress — CA3080 OTA Dyna Comp-style compressor. MXR-inspired look
  // (red box + cursive logo) recreated, not branded. Param order: Output0 Sensitivity1.
  ...
  // tag. Real pedal controls: Output0 Sensitivity1.
  P.dynacomp = { w:300, h:460, knobs:[
      {id:0,cx:.32,cy:.165,r:.082,style:'davies'},
      {id:1,cx:.68,cy:.165,r:.082,style:'davies'}],
    ...
    draw(d){ const {ctx:c,W,H,s}=d; const ink=rgb(24,20,18), m=7*s;
      ...
      rr(c,m,m,W-2*m,H-2*m,13*s); c.fillStyle=g; c.fill();
      rr(c,m,m,W-2*m,H-2*m,13*s); c.strokeStyle='rgba(0,0,0,0.4)'; c.lineWidth=2*s; c.stroke();
      textSpaced(d,.32*W,.285*H,F.barlow,9.5,ink,'OUTPUT',0.4);
      textSpaced(d,.68*W,.285*H,F.barlow,8.5,ink,'SENSITIVITY',0.2);
  ...
  // Time0 Range1 Feedback2 Drive3 Output Level4 Mix5 LFO Shape6 LFO Rate7 LFO Amount8.
  P.fm108 = { w:360,h:540,
    knobs:[
      {id:0,cx:.315,cy:.285,r:.076,style:'moog'},
      {id:2,cx:.315,cy:.585,r:.070,style:'moog'},
      {id:3,cx:.505,cy:.285,r:.052,style:'moog'},
      {id:4,cx:.505,cy:.440,r:.052,style:'moog'},
      {id:5,cx:.505,cy:.585,r:.052,style:'moog'},
      {id:6,cx:.710,cy:.270,r:.075,style:'moog'},
      {id:7,cx:.710,cy:.475,r:.066,style:'moog'},
      {id:8,cx:.710,cy:.650,r:.066,style:'moog'}],
    sw3:[{id:1,cx:.315,cy:.445,hw:32,hh:22,hidden:true,two:true}],
    tick:rgb(150,152,158), ptr:rgb(238,240,244),
    draw(d,values){ const {ctx:c,W,H,s}=d; foogBody(d); const wt=rgb(226,228,232), dim=rgb(155,158,164);
      ...
      textC(d,.675*W,.052*H,F.crete,13,wt,'FM108');
      textC(d,.675*W,.080*H,F.barlow,7,wt,'CLUSTERFLUX');

      const panel=(x,y,w,h,title)=>{
        rr(c,x*W,y*H,w*W,h*H,8*s); c.strokeStyle=wt; c.lineWidth=1.5*s; c.stroke();
        rr(c,x*W,y*H,w*W,.045*H,6*s); c.fillStyle=rgb(240,240,238); c.fill();
        textC(d,(x+w*.5)*W,(y+.027)*H,F.barlow,13,rgb(25,25,28),title);
      };
      panel(.205,.120,.235,.600,'DELAY');
      panel(.610,.120,.235,.600,'LFO');

      textC(d,.315*W,.178*H,F.barlow,10,wt,'TIME');
      textC(d,.315*W,.390*H,F.barlow,10,wt,'RANGE');
      textC(d,.255*W,.430*H,F.barlow,6.8,wt,'FLANGE');
      textC(d,.375*W,.430*H,F.barlow,6.8,wt,'CHORUS');
      textC(d,.315*W,.525*H,F.barlow,9.5,wt,'FEEDBACK');
      textC(d,.258*W,.675*H,F.barlow,7.8,wt,'-∞');
      textC(d,.374*W,.675*H,F.barlow,7.8,wt,'+∞');

      textC(d,.505*W,.210*H,F.barlow,10.5,wt,'DRIVE');
      textC(d,.505*W,.370*H,F.barlow,9.5,wt,'OUTPUT');
      textC(d,.505*W,.397*H,F.barlow,9.5,wt,'LEVEL');
      textC(d,.505*W,.520*H,F.barlow,10.5,wt,'MIX');

      textC(d,.710*W,.178*H,F.barlow,9.5,wt,'SHAPE');
      textC(d,.710*W,.405*H,F.barlow,9.5,wt,'RATE');
      textC(d,.710*W,.590*H,F.barlow,9.5,wt,'AMOUNT');
      ledDot(d,.618*W,.408*H,true,220,70,65);
      ...
      // Range switch is drawn manually; sw3 only provides the invisible hit-area.
      const on = values && values[1] > 0.5;
      rr(c,.253*W,.437*H,.124*W,.030*H,3*s); c.fillStyle=rgb(110,26,22); c.fill();
      rr(c,(on?.315:.253)*W,.431*H,.062*W,.042*H,3*s); c.fillStyle=rgb(226,58,46); c.fill();
      rr(c,(on?.315:.253)*W,.431*H,.062*W,.042*H,3*s); c.strokeStyle=rgb(60,16,12); c.lineWidth=s; c.stroke();
    } };
  P.fm107 = P.fm108;
  ...
  P.fm104 = { w:340,h:520, knobs:[
      {id:2,cx:.245,cy:.200,r:.066,style:'moog'},   // TIME
      {id:3,cx:.245,cy:.475,r:.058,style:'moog'},   // FEEDBACK
      {id:0,cx:.50,cy:.160,r:.042,style:'moog'},    // DRIVE
      {id:1,cx:.50,cy:.330,r:.042,style:'moog'},    // OUTPUT LEVEL
      {id:4,cx:.50,cy:.490,r:.042,style:'moog'},    // MIX
      {id:8,cx:.755,cy:.200,r:.054,style:'moog'},   // LFO WAVEFORM
      {id:6,cx:.755,cy:.390,r:.056,style:'moog'},   // LFO RATE
      {id:7,cx:.755,cy:.565,r:.056,style:'moog'}],  // LFO AMOUNT
    switches:[{id:5,cx:.245,cy:.330,hs:.036}],
    tick:rgb(150,152,158), ptr:rgb(238,240,244),
    draw(d,values){ const {ctx:c,W,H,s}=d; foogBody(d); const wt=rgb(230,232,236), dim=rgb(150,152,158);
      ...
      panel(.085,.105,.32,.53,'DELAY');
      panel(.595,.105,.32,.53,'LFO');
      // labels ABOVE their knobs (like the photo)
      textSpaced(d,.245*W,.140*H,F.barlow,8.5,wt,'TIME',0.25);
      textSpaced(d,.245*W,.412*H,F.barlow,8,wt,'FEEDBACK',0.2);
      textSpaced(d,.50*W,.103*H,F.barlow,8.5,wt,'DRIVE',0.25);
      textSpaced(d,.50*W,.258*H,F.barlow,7.5,wt,'OUTPUT',0.2);
      textSpaced(d,.50*W,.278*H,F.barlow,7.5,wt,'LEVEL',0.2);
      textSpaced(d,.50*W,.434*H,F.barlow,8.5,wt,'MIX',0.25);
      textSpaced(d,.755*W,.328*H,F.barlow,8,wt,'RATE',0.25);
      textSpaced(d,.755*W,.502*H,F.barlow,8,wt,'AMOUNT',0.2);
      // TIME dial marks (.1-.6, photo)
      textC(d,.170*W,.157*H,F.barlow,6,dim,'.2'); textC(d,.320*W,.157*H,F.barlow,6,dim,'.3');
      textC(d,.160*W,.252*H,F.barlow,6,dim,'.1'); textC(d,.330*W,.252*H,F.barlow,6,dim,'.6');
      // FEEDBACK 0-10
      textC(d,.175*W,.437*H,F.barlow,6,dim,'4'); textC(d,.315*W,.437*H,F.barlow,6,dim,'6');
      textC(d,.168*W,.528*H,F.barlow,6,dim,'0'); textC(d,.322*W,.528*H,F.barlow,6,dim,'10');
      // WAVEFORM glyphs hugging the wave knob (sine/tri/square/S&H)
      textC(d,.678*W,.163*H,F.barlow,7,dim,'∿'); textC(d,.832*W,.163*H,F.barlow,7,dim,'⊓');
      textC(d,.672*W,.238*H,F.barlow,7,dim,'∧'); textC(d,.838*W,.238*H,F.barlow,7,dim,'⌁');
      // RATE .05-50 (photo)
      textC(d,.680*W,.352*H,F.barlow,6,dim,'.8'); textC(d,.830*W,.352*H,F.barlow,6,dim,'3');
      textC(d,.668*W,.443*H,F.barlow,6,dim,'.05'); textC(d,.842*W,.443*H,F.barlow,6,dim,'50');
      // AMOUNT 0-10
      textC(d,.685*W,.527*H,F.barlow,6,dim,'4'); textC(d,.825*W,.527*H,F.barlow,6,dim,'6');
      textC(d,.678*W,.618*H,F.barlow,6,dim,'0'); textC(d,.832*W,.618*H,F.barlow,6,dim,'10');
      // SHORT/LONG white rocker (id5), photo-style
      const rv=(values&&values[5]!=null)?values[5]:0;
      textSpaced(d,.180*W,.301*H,F.barlow,6.6,rv<0.5?wt:dim,'SHORT',0.15);
      textSpaced(d,.312*W,.301*H,F.barlow,6.6,rv>=0.5?wt:dim,'LONG',0.15);
      rr(c,.170*W,.318*H,.150*W,.026*H,3*s); c.fillStyle=rgb(60,60,64); c.fill();
      rr(c,(rv>=0.5?.245:.170)*W,.313*H,.075*W,.036*H,3*s); c.fillStyle=rgb(238,238,236); c.fill();
      rr(c,(rv>=0.5?.245:.170)*W,.313*H,.075*W,.036*H,3*s); c.strokeStyle=rgb(40,40,44); c.lineWidth=s; c.stroke();
      // LED row (photo: TIME red / LEVEL yellow / BYPASS green)
      const led=(x,lbl,r2,g2,b2)=>{ ledDot(d,x*W,.672*H,true,r2,g2,b2); textSpaced(d,x*W,.700*H,F.barlow,6.5,wt,lbl,0.2); };
      led(.245,'TIME',225,60,60); led(.50,'LEVEL',215,200,70); led(.755,'BYPASS',90,200,110);
      // stomps + foog logo
      textSpaced(d,.28*W,.752*H,F.barlow,8.5,wt,'TAP TEMPO',0.3);
      textSpaced(d,.72*W,.752*H,F.barlow,8.5,wt,'BYPASS',0.3);
      footRound(d,W*.28,H*.815,15*s);
      footRound(d,W*.72,H*.815,15*s);
      textC(d,.50*W,.930*H,F.crete,24,wt,'foog'); } };
  ...
  // POWER label, 'NYR' logo box + 'flanger' tag, side jacks.
  // Real M117 controls: Manual0 Width1 Speed2 Regen3.
  P.n117rflanger = { w:460,h:320, knobs:[
      {id:0,cx:.18,cy:.255,r:.066,style:'davies'},
      {id:1,cx:.39,cy:.255,r:.066,style:'davies'},
      {id:2,cx:.61,cy:.255,r:.066,style:'davies'},
      {id:3,cx:.82,cy:.255,r:.066,style:'davies'}],
    ptr:rgb(244,244,240),
    draw(d){ const {ctx:c,W,H,s}=d; const lt=rgb(228,230,234), m=7*s;
      ...
      // POWER label + real knob labels
      textSpaced(d,.50*W,.105*H,F.barlow,7.5,lt,'POWER',1.2);
      textSpaced(d,.18*W,.435*H,F.barlow,8.5,lt,'MANUAL',0.4);
      textSpaced(d,.39*W,.435*H,F.barlow,9.5,lt,'WIDTH',0.4);
      textSpaced(d,.61*W,.435*H,F.barlow,9.5,lt,'SPEED',0.4);
      textSpaced(d,.82*W,.435*H,F.barlow,9.5,lt,'REGEN',0.4);
      // red LED
      ledDot(d,W*.50,H*.555,true,224,52,46);
  ...
  P.deluxeservant = { w:480,h:360, knobs:[
    ...
    draw(d){ const {ctx:c,W,H,s}=d; const m=7*s, ink=rgb(30,30,34), wt=rgb(234,236,240);
      ...
      // real knob labels to the LEFT of each knob, vertically centred
      textC(d,.71*W,.275*H,F.barlow,10.5,ink,'RATE','right');
      textC(d,.71*W,.485*H,F.barlow,10.5,ink,'RANGE','right');
      textC(d,.71*W,.695*H,F.barlow,10.5,ink,'COLOR','right');
      textC(d,.71*W,.875*H,F.barlow,8.5,ink,'MATRIX','right');
  ...
  // Bender Fumble Bass — Fender Rumble Bass (1995) all-tube dual-channel head
  // (parody). Blonde tweed tolex, oxblood grille, dark control panel: channel A
  // (Volume/Treble/Bass/Middle + Mid switch), A/B input jacks + Select/Channel,
  // channel B (Volume/Treble/Bass/Middle + Mid switch), Mix. ids 0..10:
  //  0 A Volume 1 A Treble 2 A Bass 3 A Middle 4 A Mid | 5 B Volume 6 B Treble
  //  7 B Bass 8 B Middle 9 B Mid | 10 Mix.
  P.benderfumble800 = { w:920, h:330,
    knobs:[
      {id:0,cx:.070,cy:.185,r:.026,style:'cream'},   // A Volume
      {id:1,cx:.130,cy:.185,r:.026,style:'cream'},   // A Treble
      {id:2,cx:.190,cy:.185,r:.026,style:'cream'},   // A Bass
      {id:3,cx:.250,cy:.185,r:.026,style:'cream'},   // A Middle
      {id:5,cx:.620,cy:.185,r:.026,style:'cream'},   // B Volume
      {id:6,cx:.680,cy:.185,r:.026,style:'cream'},   // B Treble
      {id:7,cx:.740,cy:.185,r:.026,style:'cream'},   // B Bass
      {id:8,cx:.800,cy:.185,r:.026,style:'cream'},   // B Middle
      {id:10,cx:.875,cy:.185,r:.026,style:'cream'}], // Mix
    switches:[{id:4,cx:.305,cy:.185,hs:.012,dark:true},{id:9,cx:.560,cy:.185,hs:.012,dark:true}],
    names:['A Volume','A Treble','A Bass','A Middle','A Mid Cut','B Volume','B Treble','B Bass','B Middle','B Mid Cut','Mix'],
    tick:rgb(120,112,96), ptr:rgb(236,230,212),
    draw(d,vals){ const {ctx:c,W,H,s}=d; const ink=rgb(228,222,206), dim=rgb(150,144,130);
      const lab=(cx,y,sz,t,col,al)=>textC(d,cx*W,y*H,F.barlow,sz,col||ink,t,al);
      // blonde tweed tolex
      c.fillStyle=rgb(196,176,130); c.fillRect(0,0,W,H);
      c.fillStyle=rgb(176,156,112); c.globalAlpha=.25; for(let y=0;y<H;y+=6) c.fillRect(0,y,W,2); c.globalAlpha=1;
      // oxblood grille cloth (lower body)
      const gy=H*0.36, gh=H-gy-10; rr(c,10,gy,W-20,gh,6); c.fillStyle=rgb(74,30,34); c.fill();
      c.fillStyle=rgb(40,16,20); c.globalAlpha=.35; for(let x=12;x<W-12;x+=5) c.fillRect(x,gy,1.4,gh); c.globalAlpha=1;
      rr(c,10,gy,W-20,gh,6); c.strokeStyle=rgb(150,134,98); c.lineWidth=2; c.stroke();
      // dark control panel across the top
      const ph=H*0.30; rr(c,10,8,W-20,ph,5);
      const pg=c.createLinearGradient(0,8,0,8+ph); pg.addColorStop(0,rgb(64,54,46)); pg.addColorStop(1,rgb(36,30,26));
      c.fillStyle=pg; c.fill(); rr(c,10,8,W-20,ph,5); c.strokeStyle=rgb(96,84,68); c.lineWidth=1.4; c.stroke();
      // channel labels + knob labels
      lab(.160,.045,10,'CHANNEL A',rgb(206,200,184)); lab(.710,.045,10,'CHANNEL B',rgb(206,200,184));
      [[.070,'VOLUME'],[.130,'TREBLE'],[.190,'BASS'],[.250,'MIDDLE'],
       [.620,'VOLUME'],[.680,'TREBLE'],[.740,'BASS'],[.800,'MIDDLE'],[.875,'MIX']].forEach(k=>lab(k[0],.30,10,k[1]));
      lab(.305,.30,9,'MID'); lab(.560,.30,9,'MID');
      // decorative SELECT / CHANNEL switches (routing, drawn for the 1:1 look)
      [[.350,'SELECT'],[.515,'CHAN']].forEach(sw=>{ const x=sw[0]*W,y=.185*H;
        rr(c,x-6,y-9,12,18,2); c.fillStyle=rgb(24,20,18); c.fill();
        rr(c,x-4.5,y-3.5,9,7,1.5); c.fillStyle=rgb(150,144,132); c.fill(); lab(sw[0],.30,8,sw[1]); });
      // input jacks (nudged slightly left)
      [[.400,'A IN'],[.440,'A/B'],[.480,'B IN']].forEach(j=>{ const x=j[0]*W,y=.185*H;
        c.beginPath();c.arc(x,y,7,0,7);c.fillStyle=rgb(28,24,22);c.fill();c.strokeStyle=rgb(150,144,130);c.lineWidth=1.4;c.stroke();
        c.beginPath();c.arc(x,y,2.6,0,7);c.fillStyle=rgb(14,12,10);c.fill(); lab(j[0],.30,8,j[1]); });
      // power / channel LED
      c.beginPath();c.arc(.585*W,.10*H,4,0,7);c.fillStyle=rgb(232,46,32);c.fill();c.strokeStyle=rgb(96,28,24);c.lineWidth=1;c.stroke();
      // ── big chrome "Bender" script (diagonal), lower-left on the grille — same style as Bender Deluxe ──
      c.save(); c.translate(10+(W-20)*0.175, gy+gh*0.40); c.rotate(-0.16); c.transform(1,0,-0.18,1,0,0); c.textAlign='center'; c.textBaseline='middle';
      setFont(d,F.ink,60);
      c.fillStyle='rgba(0,0,0,0.30)'; c.fillText('Bender',2.5*s,3*s);
      c.lineWidth=2.8*s; c.strokeStyle=rgb(40,42,46); c.strokeText('Bender',0,0);
      const blg=c.createLinearGradient(0,-24*s,0,24*s); blg.addColorStop(0,rgb(252,253,255)); blg.addColorStop(0.45,rgb(214,218,224)); blg.addColorStop(0.55,rgb(176,180,186)); blg.addColorStop(1,rgb(150,154,160));
      c.fillStyle=blg; c.fillText('Bender',0,0);
      c.restore();
      // ── "Fumble Bass" diagonal script to the RIGHT of the Mix knob (on the panel) ──
      c.save(); c.translate(.940*W, .175*H); c.rotate(-0.16); c.transform(1,0,-0.16,1,0,0); c.textAlign='center'; c.textBaseline='middle';
      setFont(d,F.ink,18); c.fillStyle=rgb(238,232,214); c.fillText('Fumble',0,-8*s);
      setFont(d,F.ink,14); c.fillText('Bass',7*s,8*s);
      c.restore(); } };
  ...
  // Lovolt 100 (BASS slot) — copies the nicer Lovolt DR103 face (Custom Hiwatt 100):
  // black tolex head, white piping, LOVOLT badge, control panel with Normal/Brilliant
  // input jacks (jumpered cable shown — the bass build runs both channels) + 7 knobs
  // (0 NormalVol 1 BrightVol 2 Bass 3 Treble 4 Middle 5 Presence 6 Master). No input
  // jumper switch / cab-sim here (the bass plugin has only ids 0..6).
  P.lovolt100 = { w:1560, h:600, ptr:rgb(238,239,242),
    knobs:[
    ...
    draw(d,vals){ const {ctx:c,W,H,s}=d;
      ...
      // ── INPUTS: Normal (Hi/Lo) + Brilliant (Hi/Lo) + jumper cable (both channels) ──
      ...
      jumper(nx,jLo,bx2,jHi); plug(nx,jHi);   // bass: both channels jumpered
      textSpaced(d,.085*W,py+ph*.95,F.barlow,6,rgb(150,150,156),'JUMPERED',0.04);
      // ── knob labels ──
      const lbl=(cx2,t,sz)=>textSpaced(d,cx2*W,lblY,F.barlow,sz||8,ink,t,0.02);
      lbl(.150,'NORMAL VOL',9.5); lbl(.242,'BRIGHT VOL',9.5); lbl(.333,'BASS',11); lbl(.425,'TREBLE',10.5);
      lbl(.517,'MIDDLE',10.5); lbl(.608,'PRESENCE',10); lbl(.700,'MASTER VOL',9.5);
      textSpaced(d,.390*W,py+ph*.13,F.bebas,15,inkF,'CUSTOM LOVOLT 100 BASS',0.08);
      textSpaced(d,.390*W,py+ph*.95,F.barlow,7,inkF,'MADE IN THE CUSTOM WORKSHOPS',0.04);
  ...
  P.boxac30 = { w:1400, h:560, ptr:rgb(240,237,230),
    ...
    knobs:[
      {id:0,cx:.208,cy:.738,r:.019,style:'tweed'},   // NORMAL VOLUME
      {id:1,cx:.265,cy:.738,r:.019,style:'tweed'},   // TOP BOOST VOLUME (RS Gain)
      {id:2,cx:.312,cy:.738,r:.019,style:'tweed'},   // TOP BOOST TREBLE (RS Treble)
      {id:3,cx:.359,cy:.738,r:.019,style:'tweed'},   // TOP BOOST BASS   (RS Bass)
      {id:4,cx:.423,cy:.738,r:.019,style:'tweed'},   // REVERB TONE
      {id:5,cx:.470,cy:.738,r:.019,style:'tweed'},   // REVERB LEVEL
      {id:6,cx:.534,cy:.738,r:.019,style:'tweed'},   // TREMOLO SPEED
      {id:7,cx:.581,cy:.738,r:.019,style:'tweed'},   // TREMOLO DEPTH
      {id:8,cx:.645,cy:.738,r:.019,style:'tweed'},   // MASTER TONE CUT  (RS Pres inv)
      {id:9,cx:.692,cy:.738,r:.019,style:'tweed'} ], // MASTER VOLUME
    ...
    sw3:[{id:10,cx:.1555,cy:.753,hw:34,hh:34,hidden:true}],
    draw(d,vals){ const {ctx:c,W,H,s}=d;
      ...
      const py=H*.58, ph=H*.36, px=W*.115, pw=W*.77;
      const lblY=py+ph*.69;
      ...
      const sideHandle=(hx)=>{ const hy0=H*.14, hy1=H*.57;
        bolt(hx,hy0-9*s,3.4*s); bolt(hx,hy1+9*s,3.4*s);
        rr(c,hx-13*s,hy0,26*s,hy1-hy0,11*s); c.fillStyle=rgb(17,17,18); c.fill();
        c.strokeStyle=rgb(3,3,4); c.lineWidth=1.2*s; c.stroke();
        c.save(); rr(c,hx-13*s,hy0,26*s,hy1-hy0,11*s); c.clip();
        c.strokeStyle='rgba(255,255,255,0.10)'; c.lineWidth=1*s;
        for(let yy=hy0-22*s; yy<hy1+22*s; yy+=9*s){
          c.beginPath(); c.moveTo(hx-13*s,yy); c.lineTo(hx+13*s,yy+26*s); c.stroke();
          c.beginPath(); c.moveTo(hx+13*s,yy); c.lineTo(hx-13*s,yy+26*s); c.stroke(); }
        c.restore();
      ...
      sideHandle(.048*W); sideHandle(.952*W);
      ...
      bolt(px+8*s,py+8*s,2.4*s); bolt(px+pw-8*s,py+8*s,2.4*s);
      bolt(px+8*s,py+ph-8*s,2.4*s); bolt(px+pw-8*s,py+ph-8*s,2.4*s);
      ...
      const jyH=py+ph*.34, jyL=py+ph*.62, ijN=.142*W, ijT=.169*W;
      c.save(); c.translate(.124*W,jyH); c.rotate(-Math.PI/2); textSpaced(d,0,0,F.barlow,6.5,ink,'HIGH',0.04); c.restore();
      c.save(); c.translate(.124*W,jyL); c.rotate(-Math.PI/2); textSpaced(d,0,0,F.barlow,6.5,ink,'LOW',0.04); c.restore();
      ...
      textSpaced(d,.148*W,py+ph+10*s,F.barlow,7,gold,mode,0.06);
      // NORMAL channel volume
      box(.184,.234,'NORMAL'); lbl(.208,'VOLUME');
      // TOP BOOST channel (volume + treble + bass)
      box(.241,.383,'TOP BOOST'); lbl(.265,'VOLUME'); lbl(.312,'TREBLE'); lbl(.359,'BASS');
      // REVERB (tone + level)
      box(.400,.494,'REVERB'); lbl(.423,'TONE'); lbl(.470,'LEVEL');
      // TREMOLO (speed + depth)
      box(.511,.605,'TREMOLO'); lbl(.534,'SPEED'); lbl(.581,'DEPTH');
      // MASTER (tone cut + volume)
      box(.622,.716,'MASTER'); lbl(.645,'TONE CUT'); lbl(.692,'VOLUME');
      // STANDBY + POWER (real switches, non-audio) + status jewels
      const swY=py+ph*.42;
      batToggle(d,.759*W,swY,9*s,true); batToggle(d,.811*W,swY,9*s,true);
      textSpaced(d,.759*W,py+ph*.13,F.barlow,8.5,ink,'STANDBY',0.04); textSpaced(d,.811*W,py+ph*.13,F.barlow,8.5,ink,'POWER',0.04);
      textSpaced(d,.759*W,lblY,F.barlow,6.5,ink,'HT  ON',0.03); textSpaced(d,.811*W,lblY,F.barlow,6.5,ink,'OFF  ON',0.03);
      ledDot(d,.776*W,swY+ph*.20,true,70,200,90); ledDot(d,.828*W,swY+ph*.20,true,212,60,52);
  ...
  P.bendersupernova22 = { w:1520, h:600, ptr:rgb(238,238,234),
    ...
    draw(d,vals){ const {ctx:c,W,H,s}=d;
      ...
      rr(c,gx,gy,gw,gh,6*s); c.fillStyle=rgb(172,173,177); c.fill();
      c.save(); rr(c,gx,gy,gw,gh,6*s); c.clip();
      // tejido Fender: trama horizontal + costillas verticales + sparkle sutil
      c.strokeStyle='rgba(20,20,24,0.50)'; c.lineWidth=1.2*s;
      for(let yy=gy;yy<gy+gh;yy+=3.2*s){ c.beginPath(); c.moveTo(gx,yy); c.lineTo(gx+gw,yy); c.stroke(); }
      c.strokeStyle='rgba(235,237,240,0.40)'; c.lineWidth=1.6*s;
      for(let xx=gx;xx<gx+gw;xx+=6.4*s){ c.beginPath(); c.moveTo(xx,gy); c.lineTo(xx,gy+gh); c.stroke(); }
      c.fillStyle='rgba(0,0,0,0.05)';
      for(let xx=gx;xx<gx+gw;xx+=12.8*s){ c.fillRect(xx,gy,6.4*s,gh); }
      c.fillStyle='rgba(255,255,255,0.55)';
      for(let i=0;i<260;i++){ const hx2=Math.abs(Math.sin(i*127.1)*43758.5453)%1, hy2=Math.abs(Math.sin(i*311.7)*268.5453)%1;
        c.fillRect(gx+hx2*gw, gy+hy2*gh, 1.1*s, 1.1*s); }
      ...
      // ── script "Bender" cromado en diagonal sobre la tela (como la foto) ──
      c.save(); c.translate(.185*W, gy+gh*.50); c.rotate(-0.30);
      c.textAlign='center'; c.textBaseline='middle';
      const big=c.createLinearGradient(0,-30*s,0,30*s);
      big.addColorStop(0,rgb(250,251,253)); big.addColorStop(.45,rgb(214,218,224)); big.addColorStop(.55,rgb(150,154,162)); big.addColorStop(1,rgb(226,230,236));
      c.font=`italic 800 ${Math.round(56*s)}px ${F.ink}`;
      c.lineWidth=7*s; c.lineJoin='round'; c.strokeStyle=rgb(10,10,12); c.strokeText('Bender',0,0);
      c.fillStyle=big; c.fillText('Bender',0,0);
  ...
  //    con vent central (valvulas naranjas tras las ranuras) y placa SILLA;
  //    panel negro SIMPLIFICADO: selector CHANNEL (LEDs verde/naranja/rojo) +
  //    UN banco Gain/Treble/Mid/Bass/Presence/Master + MODE cuyo id salta al
  //    banco del canal activo (C1=3..9, C2=10..16, C3=17..23, stride 7),
  //    Output, Rectifier (Spongy/Bold). Perillas domo cromo 'recto'.
  P.dualrect = { w:1700, h:680, ptr:rgb(238,240,244),
    ...
    knobs:[
      {id:0, cx:.200,cy:.720,r:.020,style:'recto',select:3},   // CHANNEL
      {id:9, cx:.268,cy:.720,r:.018,style:'recto',select:2},   // MODE (id/select dinamicos)
      {id:3, cx:.375,cy:.720,r:.019,style:'recto'},            // GAIN    (banco dinamico)
      {id:4, cx:.440,cy:.720,r:.019,style:'recto'},            // TREBLE
      {id:5, cx:.505,cy:.720,r:.019,style:'recto'},            // MID
      {id:6, cx:.570,cy:.720,r:.019,style:'recto'},            // BASS
      {id:7, cx:.635,cy:.720,r:.019,style:'recto'},            // PRESENCE
      {id:8, cx:.700,cy:.720,r:.019,style:'recto'},            // MASTER
      {id:1, cx:.778,cy:.720,r:.019,style:'recto'},            // OUTPUT
      {id:2, cx:.843,cy:.720,r:.017,style:'recto',select:2} ], // RECTIFIER Spongy/Bold
    draw(d,vals){ const {ctx:c,W,H,s}=d; const wht=rgb(232,234,238), grey=rgb(168,170,176);
      // ── remapeo del banco al canal activo ──
      const chv=Math.round(((vals&&vals[0]!=null?vals[0]:0))*2);      // 0 verde / 1 naranja / 2 rojo
      const base=3+chv*7, KN=P.dualrect.knobs;
      for(let i=0;i<6;i++) KN[2+i].id=base+i;
      KN[1].id=base+6; KN[1].select=(chv===0?2:3);
      // ── fondo cuero negro ──
      const bgr=c.createLinearGradient(0,0,0,H); bgr.addColorStop(0,rgb(24,24,26)); bgr.addColorStop(1,rgb(10,10,12));
      c.fillStyle=bgr; c.fillRect(0,0,W,H);
      c.save(); c.globalAlpha=0.06; c.strokeStyle=rgb(140,140,144); c.lineWidth=1;
      for(let i=0;i<380;i++){ const x=(i*131)%W, y=(i*71)%H; c.beginPath(); c.arc(x,y,1.8*s,0,7); c.stroke(); } c.restore();
      const bolt=(x,y,r)=>{ r=r||2.8*s; const g=c.createRadialGradient(x-r*0.3,y-r*0.3,r*0.15,x,y,r);
        g.addColorStop(0,rgb(200,202,206)); g.addColorStop(1,rgb(92,94,98));
        c.beginPath(); c.arc(x,y,r,0,7); c.fillStyle=g; c.fill(); c.strokeStyle=rgb(44,46,50); c.lineWidth=0.7*s; c.stroke(); };
      // ── PLACA DE DIAMANTE cromada ──
      const dy0=H*.06, dh=H*.45, dx0=W*.062, dw=W*.876;
      const pg0=c.createLinearGradient(0,dy0,0,dy0+dh);
      pg0.addColorStop(0,rgb(226,229,234)); pg0.addColorStop(.35,rgb(196,200,207));
      pg0.addColorStop(.65,rgb(168,172,180)); pg0.addColorStop(1,rgb(142,146,154));
      rr(c,dx0,dy0,dw,dh,5*s); c.fillStyle=pg0; c.fill();
      c.save(); rr(c,dx0,dy0,dw,dh,5*s); c.clip();
      // manchas de reflejo del metal
      for(const [mx2,al] of [[.30,.10],[.62,.14],[.85,.08]]){
        const rg=c.createRadialGradient(W*mx2,dy0+dh*.4,10*s,W*mx2,dy0+dh*.4,dh*1.1);
        rg.addColorStop(0,'rgba(70,74,82,'+al+')'); rg.addColorStop(1,'rgba(0,0,0,0)');
        c.fillStyle=rg; c.fillRect(dx0,dy0,dw,dh); }
      // estrellas 4 puntas (tread) tresbolillo
      const sp=44*s;
      for(let ry=dy0+10*s; ry<dy0+dh; ry+=sp*0.62){ const off=((Math.round((ry-dy0)/(sp*0.62)))%2)*sp*0.5;
        for(let rx3=dx0+8*s+off; rx3<dx0+dw; rx3+=sp){
          c.save(); c.translate(rx3,ry); c.rotate(0.5);
          const st=(l,w2)=>{ c.beginPath(); c.moveTo(-l,0); c.lineTo(0,-w2); c.lineTo(l,0); c.lineTo(0,w2); c.closePath(); };
          st(11*s,3.2*s); c.fillStyle='rgba(30,32,38,0.85)'; c.fill();
          c.save(); c.translate(-1*s,-1*s); st(10*s,2.6*s); c.fillStyle='rgba(255,255,255,0.55)'; c.fill(); c.restore();
          st(7*s,2*s); c.fillStyle='rgba(90,94,102,0.9)'; c.fill();
          c.restore(); } }
      c.restore();
      rr(c,dx0,dy0,dw,dh,5*s); c.strokeStyle=rgb(70,72,78); c.lineWidth=1.6*s; c.stroke();
      bolt(dx0+9*s,dy0+9*s); bolt(dx0+dw-9*s,dy0+9*s); bolt(dx0+9*s,dy0+dh-9*s); bolt(dx0+dw-9*s,dy0+dh-9*s);
      // ── VENT central con valvulas naranjas + placa SILLA ──
      const vw=W*.44, vh=H*.14, vx=W*.5-vw/2, vy=dy0+dh*.5-vh/2;
      const vg2=c.createLinearGradient(0,vy,0,vy+vh); vg2.addColorStop(0,rgb(206,209,215)); vg2.addColorStop(1,rgb(150,153,160));
      rr(c,vx,vy,vw,vh,5*s); c.fillStyle=vg2; c.fill(); c.strokeStyle=rgb(70,72,78); c.lineWidth=1.4*s; c.stroke();
      bolt(vx+8*s,vy+vh*.5,2.4*s); bolt(vx+vw-8*s,vy+vh*.5,2.4*s);
      // ranuras con glow de valvulas
      const slotW=7.5*s, gap=13*s, sy=vy+vh*.16, sh2=vh*.68;
      c.save();
      for(let sx2=vx+16*s; sx2<vx+vw-16*s; sx2+=gap){
        if (sx2 > W*.5-W*.085 && sx2 < W*.5+W*.085) continue;   // hueco de la placa
        rr(c,sx2,sy,slotW,sh2,slotW*0.5); c.fillStyle=rgb(8,8,9); c.fill();
        const dcenter=Math.min(Math.abs(sx2-(vx+vw*0.26)),Math.abs(sx2-(vx+vw*0.74)));
        if (dcenter < vw*0.14){ const gl=1-dcenter/(vw*0.14);
          c.save(); c.globalAlpha=0.75*gl; rr(c,sx2+1.4*s,sy+sh2*0.25,slotW-2.8*s,sh2*0.55,slotW*0.4);
          const tg=c.createLinearGradient(0,sy,0,sy+sh2); tg.addColorStop(0,rgb(255,140,60)); tg.addColorStop(1,rgb(160,50,15));
          c.fillStyle=tg; c.fill(); c.restore(); } }
      c.restore();
      // placa negra SILLA + ovalo DUAL RECTO
      const pbw=W*.155, pbh=vh*1.06, pbx=W*.5-pbw/2, pby=vy+vh*.5-pbh/2;
      rr(c,pbx,pby,pbw,pbh,4*s); c.fillStyle=rgb(16,16,18); c.fill();
      c.strokeStyle=rgb(120,122,128); c.lineWidth=1.6*s; c.stroke();
      c.save(); c.globalAlpha=.22; c.strokeStyle=rgb(255,255,255); c.lineWidth=0.8*s;
      for(let yy=pby+2*s; yy<pby+pbh; yy+=2.4*s){ c.beginPath(); c.moveTo(pbx+2*s,yy); c.lineTo(pbx+pbw-2*s,yy); c.stroke(); } c.restore();
      bolt(pbx+7*s,pby+7*s,2*s); bolt(pbx+pbw-7*s,pby+7*s,2*s); bolt(pbx+7*s,pby+pbh-7*s,2*s); bolt(pbx+pbw-7*s,pby+pbh-7*s,2*s);
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      c.font=`800 ${Math.round(26*s)}px ${F.bebas}`; c.fillStyle=rgb(234,236,240);
      c.fillText('S I L L A',W*.5,pby+pbh*.36);
      // ovalo DUAL RECTO
      const ow=pbw*.78, oh=pbh*.30, ox=W*.5, oy=pby+pbh*.72;
      c.strokeStyle=rgb(200,202,208); c.lineWidth=1.4*s;
      c.beginPath(); c.ellipse(ox,oy,ow/2,oh/2,0,0,7); c.stroke();
      c.font=`700 ${Math.round(11*s)}px ${F.bebas}`; c.fillStyle=rgb(214,216,222);
      c.fillText('DUAL RECTO',ox,oy+0.5*s); c.restore();
      // ── panel de control negro ──
      const py=H*.545, ph=H*.41, px=W*.062, pw=W*.876;
      const pg=c.createLinearGradient(0,py,0,py+ph); pg.addColorStop(0,rgb(32,32,35)); pg.addColorStop(.12,rgb(22,22,25)); pg.addColorStop(1,rgb(13,13,15));
      ...
      const ky=.720*H, lbY=ky+.100*H;
      const lb=(cx2,t,sz)=>textSpaced(d,cx2*W,lbY,F.barlow,sz||9.5,wht,t,0.06);
      // POWER: bat + jewel rojo grande (boton del Recto)
      batToggle(d,.094*W,ky,.010*W,true);
      textSpaced(d,.094*W,lbY,F.barlow,8.5,wht,'POWER',0.06);
      const jx=.133*W;
      const jg=c.createRadialGradient(jx-3*s,ky-4*s,1.5*s,jx,ky,11.5*s);
      jg.addColorStop(0,rgb(255,150,120)); jg.addColorStop(.45,rgb(220,44,28)); jg.addColorStop(1,rgb(96,10,7));
      c.beginPath(); c.arc(jx,ky,11*s,0,7); c.fillStyle=jg; c.fill();
      c.strokeStyle=rgb(126,128,134); c.lineWidth=1.8*s; c.stroke();
      c.beginPath(); c.moveTo(jx,ky-5.5*s); c.lineTo(jx+4.8*s,ky+3.8*s); c.lineTo(jx-4.8*s,ky+3.8*s); c.closePath();
      c.fillStyle='rgba(255,190,160,0.85)'; c.fill();
      // CHANNEL: LEDs verde/naranja/rojo (canales Mesa) + label
      const cols=[[80,225,96],[255,150,40],[240,64,42]];
      for(let i=0;i<3;i++){ const lx=W*(.183+.017*i), ly=py+ph*.15;
        const on=(i===chv);
        c.beginPath(); c.arc(lx,ly,4.2*s,0,7);
        c.fillStyle=on?rgb(cols[i][0],cols[i][1],cols[i][2]):rgb(28,30,30); c.fill();
        c.strokeStyle=rgb(80,82,86); c.lineWidth=0.8*s; c.stroke();
        if(on){ c.save(); c.globalAlpha=.35; c.beginPath(); c.arc(lx,ly,7.5*s,0,7); c.fillStyle=rgb(cols[i][0],cols[i][1],cols[i][2]); c.fill(); c.restore(); } }
      const chNames=['CH 1 · CLEAN','CH 2 · VINTAGE','CH 3 · MODERN'];
      textSpaced(d,.20*W,py+ph*.06,F.barlow,7.5,grey,['GREEN','ORANGE','RED'][chv],0.08);
      lb(.20,'CHANNEL');
      // MODE: nombre del modo activo del canal
      const modeV=(vals&&vals[base+6]!=null?vals[base+6]:0);
      const modeName=chv===0?(modeV>0.5?'PUSHED':'CLEAN'):(modeV<0.25?'RAW':(modeV>0.75?'MODERN':'VINTAGE'));
      textSpaced(d,.268*W,py+ph*.06,F.barlow,7.5,grey,modeName,0.08);
      lb(.268,'MODE');
      // labels del banco
      lb(.375,'GAIN'); lb(.440,'TREBLE'); lb(.505,'MID'); lb(.570,'BASS');
      lb(.635,'PRESENCE',8.5); lb(.700,'MASTER');
      lb(.778,'OUTPUT'); lb(.843,'RECTIFIER',8);
      textSpaced(d,.843*W,py+ph*.06,F.barlow,7,grey,(vals&&vals[2]!=null&&vals[2]>0.5)?'BOLD':'SPONGY',0.08);
      // separadores
      c.strokeStyle=rgb(50,52,56); c.lineWidth=1*s;
      for(const sx of [.160,.318,.742]){ c.beginPath(); c.moveTo(sx*W,py+10*s); c.lineTo(sx*W,py+ph-10*s); c.stroke(); }
      // script 'Silla 100W HEAD' (derecha, como el logo del panel real)
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      c.font=`italic 800 ${Math.round(21*s)}px ${F.anton}`; c.fillStyle=rgb(196,198,204);
      c.fillText('Silla',.915*W,ky-8*s);
      c.font=`700 ${Math.round(8.5*s)}px ${F.bebas}`; c.fillStyle=rgb(150,152,158);
      c.fillText('100W HEAD',.915*W,ky+14*s); c.restore(); } };
  ...
  // ── MARSTEN PLEXI (Marshall 1959 Super Lead JMP) — estructura Marshall 1:1
  //    con la referencia: tolex negro dominante, marco de piping dorado con
  //    linea horizontal bajo el script crema, asa negra con herrajes dorados y
  //    el panel GOLD chico inset abajo: JMP · OFF/STANDBY cromados · indicador
  //    rojo cuadrado · 6 perillas doradas con abanicos 0-10 · INPUTS 2x2.
  //    ids: 0 Presence 1 Bass 2 Middle 3 Treble 4 Volume I 5 Volume II; cable
  //    clickeable id 6 (Bright CH I -> Both puenteado -> Normal CH II).
  ...
  P.plexi = { w:1560, h:600, ptr:rgb(244,242,236),
    ...
    draw(d,vals){ const {ctx:c,W,H,s}=d;
      ...
      // ── tolex levant negro con grano ──
      const bgr=c.createLinearGradient(0,0,0,H); bgr.addColorStop(0,rgb(30,29,31)); bgr.addColorStop(0.45,rgb(22,21,23)); bgr.addColorStop(1,rgb(12,11,13));
      c.fillStyle=bgr; c.fillRect(0,0,W,H);
      c.save(); c.beginPath(); c.rect(0,0,W,H); c.clip(); c.lineWidth=1;
      c.strokeStyle='rgba(255,255,255,0.020)';
      for(let x=-H;x<W;x+=6*s){ c.beginPath(); c.moveTo(x,0); c.lineTo(x+H,H); c.stroke(); }
      ...
      for(let i=0;i<700;i++){ const hx2=Math.abs(Math.sin(i*127.1)*43758.5453)%1, hy2=Math.abs(Math.sin(i*311.7)*268.5453)%1;
        c.fillStyle=(i%2)?'rgba(255,244,224,0.05)':'rgba(0,0,0,0.28)';
        c.fillRect(hx2*W, hy2*H, 1.3*s, 1.3*s); }
      c.restore();
      const vg=c.createRadialGradient(W*.5,H*.44,H*.40,W*.5,H*.5,W*.62);
      vg.addColorStop(0,'rgba(255,244,224,0.03)'); vg.addColorStop(1,'rgba(8,5,2,0.44)');
      c.fillStyle=vg; c.fillRect(0,0,W,H);
      // tira superior (tapa del cabezal, como la referencia)
      const tg2=c.createLinearGradient(0,0,0,H*.045);
      tg2.addColorStop(0,rgb(74,66,56)); tg2.addColorStop(.5,rgb(46,42,35)); tg2.addColorStop(1,rgb(31,28,23));
      c.fillStyle=tg2; c.fillRect(0,0,W,H*.045);
      c.strokeStyle='rgba(255,240,210,0.16)'; c.lineWidth=1.2*s;
      c.beginPath(); c.moveTo(0,H*.045); c.lineTo(W,H*.045); c.stroke();
      const bolt=(x,y,r)=>{ r=r||2.6*s; const g=c.createRadialGradient(x-r*0.3,y-r*0.3,r*0.15,x,y,r);
        g.addColorStop(0,rgb(236,222,180)); g.addColorStop(1,rgb(124,102,54));
        c.beginPath(); c.arc(x,y,r,0,7); c.fillStyle=g; c.fill(); c.strokeStyle=rgb(50,40,22); c.lineWidth=0.7*s; c.stroke(); };
      // ── asa negra con herrajes dorados (arriba, centro) ──
      const hax=.425*W, hbx=.575*W, hey=H*.052;
      [hax,hbx].forEach(bx=>{ rr(c,bx-13*s,hey-8*s,26*s,16*s,3*s);
        const mg=c.createLinearGradient(0,hey-8*s,0,hey+8*s); mg.addColorStop(0,rgb(226,198,128)); mg.addColorStop(.5,rgb(186,152,84)); mg.addColorStop(1,rgb(132,102,50));
        c.fillStyle=mg; c.fill(); c.strokeStyle=rgb(60,46,22); c.lineWidth=0.9*s; c.stroke();
        bolt(bx-7*s,hey,2*s); bolt(bx+7*s,hey,2*s); });
      c.beginPath(); c.moveTo(hax+8*s,hey-3*s);
      c.bezierCurveTo(.46*W,H*.008,.54*W,H*.008,hbx-8*s,hey-3*s);
      c.lineWidth=11*s; c.lineCap='round'; c.strokeStyle=rgb(14,14,16); c.stroke();
      c.lineWidth=3*s; c.strokeStyle='rgba(255,255,255,0.10)'; c.stroke(); c.lineCap='butt';
      // ── marco de piping dorado + linea horizontal bajo el script ──
      rr(c,W*.022,H*.075,W*.956,H*.860,16*s); c.strokeStyle=gold; c.lineWidth=2.6*s; c.stroke();
      rr(c,W*.022,H*.075,W*.956,H*.860,16*s); c.strokeStyle='rgba(255,238,190,0.25)'; c.lineWidth=0.9*s; c.stroke();
      c.beginPath(); c.moveTo(W*.032,H*.425); c.lineTo(W*.968,H*.425);
      c.strokeStyle=gold; c.lineWidth=2.2*s; c.stroke();
      // ── script "Marsten" crema con sombra (centro del panel superior) ──
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      setFont(d,F.ink,86);
      c.fillStyle='rgba(0,0,0,0.55)'; c.fillText('Marsten',W*.5+2.5*s,H*.300+3.5*s);
      c.lineWidth=4.5*s; c.lineJoin='round'; c.strokeStyle=rgb(96,80,44); c.strokeText('Marsten',W*.5,H*.300);
      c.fillStyle=cream; c.fillText('Marsten',W*.5,H*.300); c.restore();
      ...
      const pg=c.createLinearGradient(0,py,0,py+ph);
      pg.addColorStop(0,rgb(230,204,140)); pg.addColorStop(.5,rgb(212,182,112)); pg.addColorStop(1,rgb(186,154,88));
      rr(c,px,py,pw,ph,5*s); c.fillStyle=pg; c.fill();
      c.save(); rr(c,px,py,pw,ph,5*s); c.clip();
      c.strokeStyle='rgba(255,255,255,0.10)'; c.lineWidth=1;
      for(let xx=px; xx<px+pw; xx+=2.4*s){ c.beginPath(); c.moveTo(xx,py); c.lineTo(xx,py+ph); c.stroke(); }
      c.fillStyle='rgba(120,90,40,0.18)'; c.fillRect(px,py,pw,3*s);
      for(const [gx2,gy2,gr2,al2] of [[.28,.25,.55,.10],[.60,.78,.6,.09],[.80,.30,.45,.08],[.10,.7,.4,.10]]){
        const gg2=c.createRadialGradient(px+pw*gx2,py+ph*gy2,2*s,px+pw*gx2,py+ph*gy2,ph*gr2);
        gg2.addColorStop(0,'rgba(96,72,34,'+al2+')'); gg2.addColorStop(1,'rgba(0,0,0,0)');
        c.fillStyle=gg2; c.fillRect(px,py,pw,ph); }
      c.fillStyle='rgba(70,50,20,0.15)'; c.fillRect(px,py+ph-4*s,pw,4*s);
      c.restore();
      ...
      const toggle=(x,up)=>{ c.beginPath(); c.arc(x,cyP,6.2*s,0,7);
        ...
        c.fillStyle=ng; c.fill(); c.strokeStyle=rgb(70,60,36); c.lineWidth=0.9*s; c.stroke();
        const ly=up?-12*s:12*s;
        c.beginPath(); c.moveTo(x,cyP); c.lineTo(x,cyP+ly);
        c.lineWidth=3.4*s; c.lineCap='round'; c.strokeStyle=rgb(198,202,208); c.stroke(); c.lineCap='butt';
      ...
      const qx=.347*W, qy=py+ph*.62;
      // placa negra "Marsten" sobre el indicador (como el panel real)
      rr(c,qx-17*s,py+ph*.16,34*s,12*s,2*s); c.fillStyle=rgb(14,12,10); c.fill();
      c.strokeStyle=rgb(150,128,74); c.lineWidth=0.8*s; c.stroke();
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      c.font=`italic 800 ${Math.round(7.5*s)}px ${F.ink}`; c.fillStyle=rgb(228,216,190);
      c.fillText('Marsten', qx, py+ph*.16+6*s); c.restore();
      c.save(); c.globalAlpha=0.45; c.beginPath(); c.arc(qx,qy,10*s,0,7);
      const qg=c.createRadialGradient(qx,qy,1.5*s,qx,qy,10*s); qg.addColorStop(0,'rgba(255,120,50,0.9)'); qg.addColorStop(1,'rgba(255,120,50,0)');
      c.fillStyle=qg; c.fill(); c.restore();
      rr(c,qx-5.5*s,qy-5.5*s,11*s,11*s,1.5*s); c.fillStyle=rgb(30,24,18); c.fill();
      rr(c,qx-4.2*s,qy-4.2*s,8.4*s,8.4*s,1*s);
      const rq=c.createLinearGradient(qx,qy-4*s,qx,qy+4*s); rq.addColorStop(0,rgb(255,170,90)); rq.addColorStop(.5,rgb(230,80,26)); rq.addColorStop(1,rgb(140,26,10));
      ...
      const jack=(jx2,jy2)=>{ c.beginPath();
        for(let i=0;i<6;i++){ const t2=i/6*Math.PI*2+0.26;
          ...
          i?c.lineTo(vx,vy):c.moveTo(vx,vy); } c.closePath();
        const hg2=c.createLinearGradient(jx2-9*s,jy2-9*s,jx2+9*s,jy2+9*s);
        hg2.addColorStop(0,rgb(226,228,232)); hg2.addColorStop(.5,rgb(140,143,148)); hg2.addColorStop(1,rgb(196,199,204));
        c.fillStyle=hg2; c.fill(); c.strokeStyle=rgb(60,56,48); c.lineWidth=0.9*s; c.stroke();
        c.beginPath(); c.arc(jx2,jy2,6.2*s,0,7); c.fillStyle=rgb(16,14,14); c.fill();
        c.strokeStyle=rgb(190,193,198); c.lineWidth=1.4*s; c.stroke();
        c.beginPath(); c.arc(jx2,jy2,2.4*s,0,7); c.fillStyle=rgb(50,48,44); c.fill(); };
      ...
      const plug=(jx,jy)=>{ rr(c,jx-4.6*s,jy-5.5*s,9.2*s,6.5*s,2*s); c.fillStyle=rgb(36,34,32); c.fill();
        ...
        c.beginPath(); c.moveTo(jx,jy+6*s); c.bezierCurveTo(jx+6*s,jy+40*s, jx-32*s,jy+50*s, jx-44*s,H*0.99);
      ...
      const jumper=(x1,y1,x2,y2)=>{ const my=Math.max(y1,y2)+15*s;
        c.beginPath(); c.arc(x1,y1,3*s,0,7); c.fillStyle=rgb(36,34,32); c.fill();
        c.beginPath(); c.arc(x2,y2,3*s,0,7); c.fillStyle=rgb(36,34,32); c.fill();
        c.beginPath(); c.moveTo(x1,y1); c.quadraticCurveTo((x1+x2)/2,my,x2,y2);
        c.lineWidth=3.8*s; c.lineCap='round'; c.strokeStyle=rgb(18,18,20); c.stroke();
  ...
  P.marstenjcm800 = { w:1560, h:600, ptr:rgb(244,242,236),
    ...
    draw(d,vals){ const {ctx:c,W,H,s}=d;
      ...
      const cyP=py+ph*.50, topY=py+ph*.10, botY=py+ph*.925, ky=.835*H;
      ...
      textSpaced(d,.117*W,topY,F.barlow,7.5,inkF,'0',0.02); textSpaced(d,.170*W,topY,F.barlow,7.5,inkF,'0',0.02);
      textSpaced(d,.117*W,py+ph*.80,F.barlow,7.5,inkF,'I',0.02); textSpaced(d,.170*W,py+ph*.80,F.barlow,7.5,inkF,'I',0.02);
      textSpaced(d,.117*W,py+ph*.92,F.barlow,8.5,ink,'POWER',0.03);
      textSpaced(d,.170*W,py+ph*.92,F.barlow,8.5,ink,'STANDBY',0.02);
      ...
      const KR=.0150*W;
      const fan=(kx)=>{ c.save(); c.textAlign='center'; c.textBaseline='middle';
        c.font=`700 ${Math.round(8.8*s)}px ${F.barlow}`; c.fillStyle=ink;
        for(let n=0;n<=10;n+=2){ const aa=ang(n/10); const rr2=KR*1.52;
          c.fillText(String(n), kx*W+rr2*Math.cos(aa), ky+rr2*Math.sin(aa)); }
        c.restore(); };
      [.400,.460,.520,.580,.640,.700].forEach(fan);
      textSpaced(d,.400*W,topY,F.barlow,8.5,ink,'PRESENCE',0.02);
      textSpaced(d,.460*W,topY,F.barlow,8.5,ink,'BASS',0.03);
      textSpaced(d,.520*W,topY,F.barlow,8.5,ink,'MIDDLE',0.02);
      textSpaced(d,.580*W,topY,F.barlow,8.5,ink,'TREBLE',0.02);
      textSpaced(d,.640*W,topY,F.barlow,8.5,ink,'MASTER',0.02);
      textSpaced(d,.700*W,topY,F.barlow,8.5,ink,'PRE-AMP',0.02);
      textSpaced(d,.640*W,botY,F.barlow,8.5,ink,'VOLUME',0.02);
      textSpaced(d,.700*W,botY,F.barlow,8.5,ink,'VOLUME',0.02);
      ...
      textSpaced(d,jx,botY,F.barlow,8,ink,'HIGH SENSITIVITY',0.02);
  ...
  // ── MARSTEN JTM45 (Marshall JTM45 MkII) — estructura 1:1 con la referencia:
  //    tolex negro dominante, marco de piping dorado con linea horizontal bajo
  //    el script crema, asa negra con herrajes dorados y el panel GOLD ANCHO
  //    inset abajo (~.19-.88): MK II · MAINS/STANDBY cromados palanca abajo ·
  //    INDICATOR rojo cuadrado · 6 perillas de ALUMINIO (PRESENCE BASS MIDDLE
  //    TREBLE HIGH TREBLE NORMAL, con LOUDNESS 1/2 debajo de las dos ultimas)
  //    con abanicos 0-10 · INPUTS 2x2 (fila top = canal High Treble, fila
  //    bottom = Normal) · placa JTM/45 a la derecha.
  //    ids: 0 Presence 1 Bass 2 Middle 3 Treble 4 Loudness1 5 Loudness2; cable
  //    clickeable id 6 (High Treble -> jumpereado -> Normal).
  P.marstenjtm45 = { w:1560, h:600, ptr:rgb(244,242,236),
    knobs:[
      {id:0,cx:.385,cy:.755,r:.0165,style:'jtmsilver'},  // PRESENCE
      {id:1,cx:.443,cy:.755,r:.0165,style:'jtmsilver'},  // BASS
      {id:2,cx:.501,cy:.755,r:.0165,style:'jtmsilver'},  // MIDDLE
      {id:3,cx:.559,cy:.755,r:.0165,style:'jtmsilver'},  // TREBLE
      {id:4,cx:.617,cy:.755,r:.0165,style:'jtmsilver'},  // HIGH TREBLE (Loudness 1)
      {id:5,cx:.675,cy:.755,r:.0165,style:'jtmsilver'} ],// NORMAL (Loudness 2)
    sw3:[{id:6,cx:.769,cy:.752,hw:64,hh:50,hidden:true}], // cable High Treble/Both/Normal
    draw(d,vals){ const {ctx:c,W,H,s}=d;
      const gold=rgb(198,170,96), ink=rgb(52,40,22), inkF='rgba(52,40,22,0.62)',
            cream=rgb(242,235,214);
      // ── tolex levant negro con grano ──
      ...
      vg.addColorStop(0,'rgba(255,244,224,0.03)'); vg.addColorStop(1,'rgba(8,5,2,0.44)');
      c.fillStyle=vg; c.fillRect(0,0,W,H);
      // tira superior (tapa del cabezal)
      const tg2=c.createLinearGradient(0,0,0,H*.045);
      tg2.addColorStop(0,rgb(74,66,56)); tg2.addColorStop(.5,rgb(46,42,35)); tg2.addColorStop(1,rgb(31,28,23));
      c.fillStyle=tg2; c.fillRect(0,0,W,H*.045);
      c.strokeStyle='rgba(255,240,210,0.16)'; c.lineWidth=1.2*s;
      ...
      const bolt=(x,y,r)=>{ r=r||2.6*s; const g=c.createRadialGradient(x-r*0.3,y-r*0.3,r*0.15,x,y,r);
        g.addColorStop(0,rgb(236,222,180)); g.addColorStop(1,rgb(124,102,54));
        c.beginPath(); c.arc(x,y,r,0,7); c.fillStyle=g; c.fill(); c.strokeStyle=rgb(50,40,22); c.lineWidth=0.7*s; c.stroke(); };
      // ── asa negra con herrajes dorados ──
      const hax=.425*W, hbx=.575*W, hey=H*.052;
      [hax,hbx].forEach(bx=>{ rr(c,bx-13*s,hey-8*s,26*s,16*s,3*s);
        const mg=c.createLinearGradient(0,hey-8*s,0,hey+8*s); mg.addColorStop(0,rgb(226,198,128)); mg.addColorStop(.5,rgb(186,152,84)); mg.addColorStop(1,rgb(132,102,50));
        c.fillStyle=mg; c.fill(); c.strokeStyle=rgb(60,46,22); c.lineWidth=0.9*s; c.stroke();
      ...
      c.beginPath(); c.moveTo(W*.032,H*.425); c.lineTo(W*.968,H*.425);
      c.strokeStyle=gold; c.lineWidth=2.2*s; c.stroke();
      // ── script "Marsten" crema con sombra ──
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      setFont(d,F.ink,86);
      c.fillStyle='rgba(0,0,0,0.55)'; c.fillText('Marsten',W*.5+2.5*s,H*.300+3.5*s);
      c.lineWidth=4.5*s; c.lineJoin='round'; c.strokeStyle=rgb(96,80,44); c.strokeText('Marsten',W*.5,H*.300);
      c.fillStyle=cream; c.fillText('Marsten',W*.5,H*.300); c.restore();
      // ── panel GOLD ANCHO inset (mas ancho que el del JMP: MK II a JTM45) ──
      const px=W*.190, pw=W*.690, py=H*.635, ph=H*.235;
      rr(c,px-4*s,py-4*s,pw+8*s,ph+8*s,7*s); c.fillStyle=rgb(5,5,6); c.fill();     // recess
      rr(c,px-4*s,py-4*s,pw+8*s,ph+8*s,7*s); c.strokeStyle=gold; c.lineWidth=1.8*s; c.stroke();
      const pg=c.createLinearGradient(0,py,0,py+ph);
      pg.addColorStop(0,rgb(230,204,140)); pg.addColorStop(.5,rgb(212,182,112)); pg.addColorStop(1,rgb(186,154,88));
      ...
      for(let xx=px; xx<px+pw; xx+=2.4*s){ c.beginPath(); c.moveTo(xx,py); c.lineTo(xx,py+ph); c.stroke(); }
      c.fillStyle='rgba(120,90,40,0.18)'; c.fillRect(px,py,pw,3*s);
      for(const [gx2,gy2,gr2,al2] of [[.24,.25,.55,.10],[.58,.78,.6,.09],[.82,.30,.45,.08],[.08,.7,.4,.10]]){
        const gg2=c.createRadialGradient(px+pw*gx2,py+ph*gy2,2*s,px+pw*gx2,py+ph*gy2,ph*gr2);
        gg2.addColorStop(0,'rgba(96,72,34,'+al2+')'); gg2.addColorStop(1,'rgba(0,0,0,0)');
        c.fillStyle=gg2; c.fillRect(px,py,pw,ph); }
      c.fillStyle='rgba(70,50,20,0.15)'; c.fillRect(px,py+ph-4*s,pw,4*s);
      c.restore();
      rr(c,px,py,pw,ph,5*s); c.strokeStyle=rgb(130,104,52); c.lineWidth=1.2*s; c.stroke();
      const topY=py+ph*.10, botY=py+ph*.88, cyP=py+ph*.51;
      // ── MK II (borde izquierdo del panel, apilado) ──
      textSpaced(d,.215*W,cyP-8*s,F.bebas,12,ink,'MK',0.10);
      textSpaced(d,.215*W,cyP+8*s,F.bebas,12,ink,'II',0.16);
      // ── toggles cromados MAINS / STANDBY (palanca ABAJO, como la foto) ──
      const toggle=(x)=>{ c.beginPath(); c.arc(x,cyP,6.2*s,0,7);
        const ng=c.createRadialGradient(x-2*s,cyP-2*s,1*s,x,cyP,6.2*s); ng.addColorStop(0,rgb(244,246,248)); ng.addColorStop(1,rgb(130,134,140));
        c.fillStyle=ng; c.fill(); c.strokeStyle=rgb(70,60,36); c.lineWidth=0.9*s; c.stroke();
        c.beginPath(); c.moveTo(x,cyP); c.lineTo(x,cyP+12*s);
        c.lineWidth=3.4*s; c.lineCap='round'; c.strokeStyle=rgb(198,202,208); c.stroke(); c.lineCap='butt';
        c.beginPath(); c.arc(x,cyP+12*s,2.6*s,0,7);
        const bg2=c.createRadialGradient(x-1*s,cyP+11*s,0.5*s,x,cyP+12*s,2.6*s); bg2.addColorStop(0,rgb(250,252,254)); bg2.addColorStop(1,rgb(150,154,160));
        c.fillStyle=bg2; c.fill(); };
      toggle(.253*W); toggle(.291*W);
      textSpaced(d,.253*W,topY,F.barlow,7,ink,'MAINS',0.03); textSpaced(d,.291*W,topY,F.barlow,7,ink,'STANDBY',0.02);
      textSpaced(d,.253*W,botY,F.barlow,6.5,inkF,'ON',0.04); textSpaced(d,.291*W,botY,F.barlow,6.5,inkF,'ON',0.04);
      // ── indicador ROJO cuadrado ──
      const qx=.327*W, qy=cyP;
      c.save(); c.globalAlpha=0.45; c.beginPath(); c.arc(qx,qy,10*s,0,7);
      const qg=c.createRadialGradient(qx,qy,1.5*s,qx,qy,10*s); qg.addColorStop(0,'rgba(255,120,50,0.9)'); qg.addColorStop(1,'rgba(255,120,50,0)');
      c.fillStyle=qg; c.fill(); c.restore();
      rr(c,qx-5.5*s,qy-5.5*s,11*s,11*s,1.5*s); c.fillStyle=rgb(30,24,18); c.fill();
      ...
      c.fillStyle=rq; c.fill();
      textSpaced(d,qx,botY,F.barlow,6.5,inkF,'INDICATOR',0.02);
      // ── abanicos 0-10 (pares) + captions de las 6 perillas ──
      const KR=.0165*W, ky=.755*H, kxs=[.385,.443,.501,.559,.617,.675];
      const fan=(kx)=>{ c.save(); c.textAlign='center'; c.textBaseline='middle';
        c.font=`700 ${Math.round(7.8*s)}px ${F.barlow}`; c.fillStyle=ink;
        for(let n=0;n<=10;n+=2){ const aa=ang(n/10); const rr2=KR*1.50;
          c.fillText(String(n), kx*W+rr2*Math.cos(aa), ky+rr2*Math.sin(aa)); }
        c.restore(); };
      kxs.forEach(fan);
      textSpaced(d,.385*W,topY,F.barlow,7.5,ink,'PRESENCE',0.02);
      textSpaced(d,.443*W,topY,F.barlow,7.5,ink,'BASS',0.03);
      textSpaced(d,.501*W,topY,F.barlow,7.5,ink,'MIDDLE',0.02);
      textSpaced(d,.559*W,topY,F.barlow,7.5,ink,'TREBLE',0.02);
      textSpaced(d,.617*W,topY,F.barlow,7.5,ink,'HIGH TREBLE',0.02);
      textSpaced(d,.675*W,topY,F.barlow,7.5,ink,'NORMAL',0.02);
      textSpaced(d,.617*W,botY,F.barlow,7.5,ink,'LOUDNESS 1',0.02);
      textSpaced(d,.675*W,botY,F.barlow,7.5,ink,'LOUDNESS 2',0.02);
      // ── INPUTS 2x2 (fila top = High Treble, fila bottom = Normal) ──
      const xA=.740*W, xB=.798*W, iy0=py+ph*.30, iy1=py+ph*.72;
      textSpaced(d,(xA+xB)/2,cyP,F.barlow,8.5,ink,'INPUTS',0.04);
      const jack=(jx2,jy2)=>{ c.beginPath();
        for(let i=0;i<6;i++){ const t2=i/6*Math.PI*2+0.26;
          const vx=jx2+9.5*s*Math.cos(t2), vy=jy2+9.5*s*Math.sin(t2);
          i?c.lineTo(vx,vy):c.moveTo(vx,vy); } c.closePath();
        ...
        c.beginPath(); c.arc(jx2,jy2,2.4*s,0,7); c.fillStyle=rgb(50,48,44); c.fill(); };
      jack(xA,iy0); jack(xB,iy0); jack(xA,iy1); jack(xB,iy1);
      textSpaced(d,xA-17*s,iy0,F.barlow,8.5,ink,'1',0); textSpaced(d,xB+17*s,iy0,F.barlow,8.5,ink,'2',0);
      textSpaced(d,xA-17*s,iy1,F.barlow,8.5,ink,'1',0); textSpaced(d,xB+17*s,iy1,F.barlow,8.5,ink,'2',0);
      // ── placa JTM / 45 (borde derecho del panel, apilado) ──
      textSpaced(d,.845*W,cyP-8*s,F.bebas,13,ink,'JTM',0.10);
      textSpaced(d,.845*W,cyP+8*s,F.bebas,13,ink,'45',0.12);
      // ── cable / puente (click: High Treble -> jumpereado -> Normal) ──
      const inp=(vals&&vals[6]!=null)?vals[6]:0.5;
      const plug=(jx,jy)=>{ rr(c,jx-4.6*s,jy-5.5*s,9.2*s,6.5*s,2*s); c.fillStyle=rgb(36,34,32); c.fill();
        const cg=c.createLinearGradient(jx-4.6*s,jy,jx+4.6*s,jy); cg.addColorStop(0,rgb(188,192,198)); cg.addColorStop(0.5,rgb(120,124,130)); cg.addColorStop(1,rgb(188,192,198));
        rr(c,jx-4.6*s,jy+1*s,9.2*s,3.6*s,1.4*s); c.fillStyle=cg; c.fill();
      ...
      const jumper=(x1,y1,x2,y2)=>{ const my=Math.max(y1,y2)+15*s;
        ...
        c.lineWidth=1.4*s; c.strokeStyle='rgba(255,255,255,0.10)'; c.stroke(); c.lineCap='butt'; };
      let mode;
      if (inp < 0.25)      { plug(xA,iy0); mode='HIGH TREBLE'; }
      else if (inp < 0.75) { jumper(xB,iy0,xA,iy1); plug(xA,iy0); mode='JUMPERED'; }
      else                 { plug(xA,iy1); mode='NORMAL'; }
      textSpaced(d,(xA+xB)/2,py-10*s,F.barlow,7.5,gold,mode,0.06); } };
  ...
  //    script 'Deezel', panel brushed. Cintas negras DIAGONALES con la perilla
  //    al extremo derecho y el label sobre la cinta (como el frente real),
  //    CH 1-4 con LED rojo (el activo encendido), escalera master a la derecha
  //    y selector rotatorio CHANNEL. Perillas estilo 'vh4' (negras lobuladas).
  ...
  P.deezelvh4 = { w:1560, h:600, ptr:rgb(238,240,244),
    ...
    draw(d,vals){ const {ctx:c,W,H,s}=d; const ink=rgb(24,24,26);
      ...
      // ── cinta negra: nace a la IZQUIERDA y sube hasta la perilla (que va al
      //    extremo derecho); el label vive en la MITAD IZQUIERDA de la cinta ──
      const strip=(kx,ky,t,len)=>{ const L=(len||0.150)*W, bh=17*s, slope=0.34;
        const x1=kx*W, y1=ky*H, x0=x1-L, y0=y1+L*slope*0.28;
        const angl=Math.atan2(y1-y0,x1-x0);
        c.save(); c.translate(x0,y0); c.rotate(angl);
        const D=Math.hypot(x1-x0,y1-y0);
        rr(c,0,-bh*0.5,D,bh,3*s); c.fillStyle=rgb(15,15,17); c.fill();
      ...
      for(let i=0;i<4;i++){ const lx=W*(.525+.093*i), ly=py+ph*.115;
        const on=(i===chv);
        c.beginPath(); c.arc(lx,ly,7.5*s,0,7); c.fillStyle=on?rgb(70,10,8):rgb(40,12,10); c.fill();
        ...
        else { c.beginPath(); c.arc(lx,ly,5.6*s,0,7); c.fillStyle=rgb(96,24,18); c.fill(); }
        c.strokeStyle=rgb(118,120,124); c.lineWidth=1.2*s; c.beginPath(); c.arc(lx,ly,7.5*s,0,7); c.stroke();
        c.font=`italic 700 ${Math.round(9.5*s)}px ${F.barlow}`; c.textAlign='left'; c.textBaseline='middle';
        c.fillStyle=ink; c.fillText(names[i],lx+11*s,ly); }
      // selector CHANNEL (izquierda)
      textSpaced(d,.125*W,py+ph*.28,F.barlow,10,ink,'CHANNEL',0.05);
      textSpaced(d,.125*W,py+ph*.965,F.barlow,7.5,rgb(88,90,94),'CLEAN · CRUNCH · MEGA · LEAD',0.02);
      // power (derecha)
      const rx=.915*W, ry=py+ph*.42;
  ...
  // ── RANEY IRONHEART (Laney IRT60H) — head de cuero negro con esquineras,
  //    rejilla expanded-metal con VALVULAS brillando en rojo detras, placa
  //    'Raney' estilo Laney (marco plateado + campo negro + logo blanco),
  //    panel negro con abanicos de numeros 0-10 por perilla, labels DEBAJO,
  //    PRE-BOOST rojo + toggle + LED, selector de canal con LEDs, seccion
  //    master y jewel POWER. Perillas estilo 'irt'.
  ...
  P.raneyironheart = { w:1660, h:620, ptr:rgb(238,240,244),
    ...
    draw(d,vals){ const {ctx:c,W,H,s}=d; const wht=rgb(232,234,238), grey=rgb(176,178,184);
      ...
      c.fillStyle=bgr; c.fillRect(0,0,W,H);
      c.save(); c.globalAlpha=0.06; c.strokeStyle=rgb(140,140,144); c.lineWidth=1;
      for(let i=0;i<420;i++){ const x=(i*127)%W, y=(i*83)%H, r0=1.4+((i*7)%14)/10;
        c.beginPath(); c.arc(x,y,r0*s,0,7); c.stroke(); } c.restore();
      const vg=c.createRadialGradient(W*.5,H*.5,H*.35,W*.5,H*.5,W*.62);
      vg.addColorStop(0,'rgba(0,0,0,0)'); vg.addColorStop(1,'rgba(0,0,0,0.42)');
      c.fillStyle=vg; c.fillRect(0,0,W,H);
      const bolt=(x,y,r)=>{ r=r||2.8*s; const g=c.createRadialGradient(x-r*0.3,y-r*0.3,r*0.15,x,y,r);
        g.addColorStop(0,rgb(190,192,196)); g.addColorStop(1,rgb(84,86,90));
        c.beginPath(); c.arc(x,y,r,0,7); c.fillStyle=g; c.fill(); c.strokeStyle=rgb(40,42,46); c.lineWidth=0.7*s; c.stroke();
        c.strokeStyle=rgb(48,50,54); c.lineWidth=0.9*s;
        c.beginPath(); c.moveTo(x-r*0.55,y); c.lineTo(x+r*0.55,y); c.stroke(); };
      // esquineras negras
      const corner=(cxx,cyy,dx,dy)=>{ const k=H*.10;
        c.beginPath(); c.moveTo(cxx,cyy+dy*k); c.lineTo(cxx,cyy); c.lineTo(cxx+dx*k,cyy);
        c.quadraticCurveTo(cxx+dx*k*0.42,cyy+dy*k*0.42,cxx,cyy+dy*k); c.closePath();
        const cg=c.createLinearGradient(cxx,cyy,cxx+dx*k*0.8,cyy+dy*k*0.8);
        cg.addColorStop(0,rgb(46,47,50)); cg.addColorStop(1,rgb(18,18,20));
        c.fillStyle=cg; c.fill(); c.strokeStyle=rgb(6,6,8); c.lineWidth=1.2*s; c.stroke();
        bolt(cxx+dx*k*0.40,cyy+dy*k*0.40,2.6*s); };
      // ── rejilla expanded-metal con valvulas ──
      const dy0=H*.065, dh=H*.44, dx0=W*.068, dw=W*.864;
      rr(c,dx0-4*s,dy0-4*s,dw+8*s,dh+8*s,6*s); c.fillStyle=rgb(26,26,29); c.fill();
      c.strokeStyle=rgb(8,8,10); c.lineWidth=1.6*s; c.stroke();
      rr(c,dx0,dy0,dw,dh,4*s); c.fillStyle=rgb(4,4,5); c.fill();
      c.save(); rr(c,dx0,dy0,dw,dh,4*s); c.clip();
      // fondo calido
      const warm=c.createLinearGradient(0,dy0,0,dy0+dh);
      warm.addColorStop(0,rgb(16,7,5)); warm.addColorStop(1,rgb(6,3,2));
      c.fillStyle=warm; c.fillRect(dx0,dy0,dw,dh);
      // VALVULAS: botellas con nucleo brillante + transformadores oscuros
      const tube=(tx,tw2,th2,glow)=>{ const ty=dy0+dh*0.98;
        if(glow){ const g=c.createRadialGradient(tx,ty-th2*0.55,2*s,tx,ty-th2*0.55,th2*0.9);
          g.addColorStop(0,'rgba(255,120,60,0.85)'); g.addColorStop(.35,'rgba(220,50,20,0.45)'); g.addColorStop(1,'rgba(0,0,0,0)');
          c.fillStyle=g; c.fillRect(tx-th2,ty-th2*1.6,th2*2,th2*1.8); }
        const bg2=c.createLinearGradient(tx-tw2,0,tx+tw2,0);
        bg2.addColorStop(0,'rgba(120,60,40,0.25)'); bg2.addColorStop(.5,'rgba(210,120,80,0.5)'); bg2.addColorStop(1,'rgba(90,45,30,0.25)');
        c.beginPath(); rr(c,tx-tw2,ty-th2,tw2*2,th2,tw2);
        c.fillStyle=bg2; c.fill();
        if(glow){ c.beginPath(); rr(c,tx-tw2*0.30,ty-th2*0.85,tw2*0.6,th2*0.55,tw2*0.3);
          c.fillStyle='rgba(255,170,110,0.9)'; c.fill(); } };
      const box=(bx2,bw2,bh2)=>{ const by2=dy0+dh*0.98;
        rr(c,bx2-bw2/2,by2-bh2,bw2,bh2,2*s);
        c.fillStyle='rgba(40,30,28,0.85)'; c.fill();
        c.strokeStyle='rgba(150,90,60,0.25)'; c.lineWidth=1.2*s; c.stroke(); };
      box(W*.205,W*.055,dh*.62); box(W*.79,W*.06,dh*.70);
      tube(W*.265,9*s,dh*.55,true); tube(W*.30,8*s,dh*.48,false);
      tube(W*.44,9*s,dh*.60,true);  tube(W*.475,8*s,dh*.50,true);
      tube(W*.545,8*s,dh*.46,false); tube(W*.71,9*s,dh*.58,true); tube(W*.745,8*s,dh*.47,false);
      // malla expanded-metal (celdas de rombo con luz y sombra)
      const mw=13*s, mh=8.5*s;
      for(let ry=dy0-mh; ry<dy0+dh+mh; ry+=mh){ const off=((Math.round((ry-dy0)/mh))%2)*mw*0.5;
        for(let rx3=dx0-mw+off; rx3<dx0+dw+mw; rx3+=mw){
          c.beginPath(); c.moveTo(rx3,ry); c.lineTo(rx3+mw*0.5,ry-mh*0.5); c.lineTo(rx3+mw,ry);
          c.lineTo(rx3+mw*0.5,ry+mh*0.5); c.closePath();
          c.strokeStyle='rgba(0,0,0,0.88)'; c.lineWidth=3.4*s; c.stroke();
          c.strokeStyle='rgba(190,190,196,0.10)'; c.lineWidth=1.1*s;
          c.beginPath(); c.moveTo(rx3,ry); c.lineTo(rx3+mw*0.5,ry-mh*0.5); c.stroke(); } }
      // vineta interna de la rejilla
      const iv=c.createRadialGradient(W*.5,dy0+dh*.5,dh*.35,W*.5,dy0+dh*.5,dw*.55);
      iv.addColorStop(0,'rgba(0,0,0,0)'); iv.addColorStop(1,'rgba(0,0,0,0.55)');
      c.fillStyle=iv; c.fillRect(dx0,dy0,dw,dh);
      c.restore();
      // tornillos del marco
      bolt(dx0+8*s,dy0+8*s); bolt(dx0+dw-8*s,dy0+8*s); bolt(dx0+8*s,dy0+dh-8*s); bolt(dx0+dw-8*s,dy0+dh-8*s);
      // ── placa Raney estilo Laney: marco plateado + campo negro + logo blanco ──
      const bw=W*.20, bh=H*.15, bx=W*.5-bw/2, by=dy0+dh*.5-bh/2;
      const mg=c.createLinearGradient(0,by,0,by+bh); mg.addColorStop(0,rgb(236,238,242)); mg.addColorStop(.5,rgb(184,186,192)); mg.addColorStop(1,rgb(130,132,138));
      rr(c,bx,by,bw,bh,7*s); c.fillStyle=mg; c.fill();
      c.strokeStyle=rgb(60,62,66); c.lineWidth=1.6*s; c.stroke();
      rr(c,bx+5*s,by+5*s,bw-10*s,bh-10*s,5*s); c.fillStyle=rgb(12,12,14); c.fill();
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      c.font=`italic 800 ${Math.round(42*s)}px ${F.anton}`;
      c.fillStyle=rgb(240,242,246); c.fillText('Raney',W*.5,by+bh*.52);
      c.font=`700 ${Math.round(8*s)}px ${F.barlow}`; c.fillStyle=rgb(190,192,198);
      c.fillText('TM',bx+bw-11*s,by+bh-10*s); c.restore();
      corner(0,0,1,1); corner(W,0,-1,1); corner(0,H,1,-1); corner(W,H,-1,-1);
      // ── panel de control negro ──
      const py=H*.545, ph=H*.42, px=W*.068, pw=W*.864;
      ...
      for(let i=0;i<6;i++){ const t=i/6*Math.PI*2+0.26; const x=12.5*s*Math.cos(t), y=12.5*s*Math.sin(t);
        i?c.lineTo(x,y):c.moveTo(x,y); }
      c.closePath(); const hg=c.createLinearGradient(-12*s,-12*s,12*s,12*s);
      hg.addColorStop(0,rgb(198,200,206)); hg.addColorStop(1,rgb(108,110,116));
      ...
      c.beginPath(); c.arc(blx,bly,4.4*s,0,7);
      c.fillStyle=bOn?rgb(240,64,42):rgb(64,16,12); c.fill(); c.strokeStyle=rgb(84,86,90); c.lineWidth=0.9*s; c.stroke();
      if(bOn){ c.save(); c.globalAlpha=.4; c.beginPath(); c.arc(blx,bly,8.5*s,0,7); c.fillStyle=rgb(255,96,66); c.fill(); c.restore(); }
      ...
      for(let i=0;i<3;i++){ const lx=W*(.207+.019*i), ly=py+ph*.15;
        ...
        c.strokeStyle=rgb(80,82,86); c.lineWidth=0.8*s; c.stroke();
        if(on){ c.save(); c.globalAlpha=.35; c.beginPath(); c.arc(lx,ly,7.5*s,0,7); c.fillStyle=rgb(col[0],col[1],col[2]); c.fill(); c.restore(); } }
      textSpaced(d,.226*W,py+ph*.10,F.barlow,7.5,rgb(170,172,178),chNames[chv],0.08);
      // separadores
      c.strokeStyle=rgb(50,52,56); c.lineWidth=1*s;
      for(const sx of [.272,.578,.655]){ c.beginPath(); c.moveTo(sx*W,py+10*s); c.lineTo(sx*W,py+ph-10*s); c.stroke(); }
      // POWER: jewel rojo facetado + switch I/0
      const rx2=.906*W;
      const jg=c.createRadialGradient(rx2-3*s,ky-4*s,1.5*s,rx2,ky,11*s);
      jg.addColorStop(0,rgb(255,150,120)); jg.addColorStop(.45,rgb(222,44,28)); jg.addColorStop(1,rgb(96,10,7));
      c.beginPath(); c.arc(rx2,ky,10.5*s,0,7); c.fillStyle=jg; c.fill();
      c.strokeStyle=rgb(120,122,128); c.lineWidth=1.8*s; c.stroke();
      c.save(); c.globalAlpha=.5; c.strokeStyle=rgb(255,180,150); c.lineWidth=0.9*s;
  ...
  // ── RONALD JC-120 (Roland JC-120H) — COPIA de la referencia: franja de
  //    panel 25% con secciones gris-verde y oliva, tira de aluminio, rejilla
  //    de cuadricula crema hasta abajo con placa Ronald (monograma de cinta
  //    dibujado), tachas y esquineras. ids: 0 Volume 1 Treble 2 Middle 3 Bass
  ...
  P.ronaldjc120 = { w:1380, h:600, ptr:rgb(244,245,248), tick:rgb(172,174,180),
    knobs:[
      {id:4,cx:.168,cy:.235,r:.0155,style:'jcsilver'},  // DISTORTION
      {id:0,cx:.220,cy:.235,r:.0155,style:'jcsilver'},  // VOLUME
      {id:1,cx:.272,cy:.235,r:.0155,style:'jcsilver'},  // HI-TREBLE
      {id:2,cx:.324,cy:.235,r:.0155,style:'jcsilver'},  // MIDDLE
      {id:3,cx:.376,cy:.235,r:.0155,style:'jcsilver'},  // BASS
      {id:5,cx:.438,cy:.235,r:.0155,style:'jcsilver'},  // REVERB
      {id:6,cx:.512,cy:.235,r:.0155,style:'jcsilver'},  // RATE
      {id:7,cx:.564,cy:.235,r:.0155,style:'jcsilver'} ],// DEPTH
    sw3:[{id:8,cx:.612,cy:.235,hw:13,hh:30}],           // palanca CHORUS
    draw(d,vals){ const {ctx:c,W,H,s}=d;
      const ink=rgb(230,231,234), inkF='rgba(230,231,234,0.65)',
            inkD=rgb(22,25,25), inkDF='rgba(22,25,25,0.7)';
      // ── tolex negro rugoso ──
      const bgr=c.createLinearGradient(0,0,0,H); bgr.addColorStop(0,rgb(36,35,36)); bgr.addColorStop(.5,rgb(27,26,27)); bgr.addColorStop(1,rgb(17,16,17));
      c.fillStyle=bgr; c.fillRect(0,0,W,H);
      c.save(); c.beginPath(); c.rect(0,0,W,H); c.clip();
      for(let i=0;i<1000;i++){ const hx2=Math.abs(Math.sin(i*127.1)*43758.5453)%1, hy2=Math.abs(Math.sin(i*311.7)*268.5453)%1;
        c.fillStyle=(i%2)?'rgba(255,255,255,0.035)':'rgba(0,0,0,0.30)';
        c.fillRect(hx2*W, hy2*H, 1.5*s, 1.5*s); }
      const vg=c.createRadialGradient(W*.5,H*.44,H*.40,W*.5,H*.5,W*.62);
      ...
      c.restore();
      // ── PANEL (25% de alto, como el real) ──
      const py=H*.095, ph=H*.25, px=W*.062, pw=W*.876;
      rr(c,px-3*s,py-3*s,pw+6*s,ph+6*s,5*s); c.fillStyle=rgb(5,5,6); c.fill();
      const pgr=c.createLinearGradient(0,py,0,py+ph); pgr.addColorStop(0,rgb(42,42,44)); pgr.addColorStop(.5,rgb(30,30,32)); pgr.addColorStop(1,rgb(21,21,23));
      rr(c,px,py,pw,ph,3.5*s); c.fillStyle=pgr; c.fill();
      c.save(); rr(c,px,py,pw,ph,3.5*s); c.clip();
      const gx0=W*.143, ox0=W*.478, ox1=W*.640;
      ...
      c.restore();
      rr(c,px,py,pw,ph,3.5*s); c.strokeStyle=rgb(80,82,86); c.lineWidth=1.1*s; c.stroke();
      const topY=py+ph*.13, botY=py+ph*.87, ky=.235*H;
      [.248,.408,.478].forEach(xx=>{ c.beginPath(); c.moveTo(xx*W,py+ph*.07); c.lineTo(xx*W,py+ph*.93);
        c.strokeStyle='rgba(28,32,32,0.35)'; c.lineWidth=1*s; c.stroke(); });
      // INPUT
      const jack=(jx2,jy2)=>{ c.beginPath(); c.arc(jx2,jy2,7.5*s,0,7); c.fillStyle=rgb(14,13,13); c.fill();
        c.strokeStyle=rgb(150,153,158); c.lineWidth=1.9*s; c.stroke();
        c.beginPath(); c.arc(jx2,jy2,2.5*s,0,7); c.fillStyle=rgb(52,52,56); c.fill(); };
      jack(.090*W,ky+5*s); jack(.120*W,ky+5*s);
      textSpaced(d,.105*W,topY,F.barlow,8.5,ink,'INPUT',0.05);
      c.strokeStyle='rgba(230,231,234,0.55)'; c.lineWidth=1*s;
      c.beginPath(); c.moveTo(.082*W,topY+7*s); c.lineTo(.082*W,topY+12*s); c.lineTo(.128*W,topY+12*s); c.lineTo(.128*W,topY+7*s); c.stroke();
      textSpaced(d,.090*W,botY,F.barlow,7.5,inkF,'HIGH',0.02);
      textSpaced(d,.120*W,botY,F.barlow,7.5,inkF,'LOW',0.02);
      // labels + numeros (GRANDES)
      const KR=.0155*W;
      const fan=(kx,offLbl)=>{ c.save(); c.textAlign='center'; c.textBaseline='middle';
        c.font=`700 ${Math.round(9.0*s)}px ${F.barlow}`; c.fillStyle=inkD;
        ...
        c.restore(); };
      fan(.168,'OFF'); fan(.220); fan(.272); fan(.324); fan(.376); fan(.438); fan(.512); fan(.564);
      textSpaced(d,.168*W,topY,F.barlow,8,inkD,'DISTORTION',0.01);
      textSpaced(d,.220*W,topY,F.barlow,8,inkD,'VOLUME',0.02);
      textSpaced(d,.272*W,topY,F.barlow,8,inkD,'HI-TREBLE',0.01);
      textSpaced(d,.324*W,topY,F.barlow,8,inkD,'MIDDLE',0.02);
      textSpaced(d,.376*W,topY,F.barlow,8,inkD,'BASS',0.02);
      textSpaced(d,.438*W,topY,F.barlow,8,inkD,'REVERB',0.02);
      textSpaced(d,.512*W,topY,F.barlow,8,inkD,'RATE',0.02);
      textSpaced(d,.564*W,topY,F.barlow,8,inkD,'DEPTH',0.02);
      c.strokeStyle='rgba(22,25,25,0.55)'; c.lineWidth=1*s;
      c.beginPath(); c.moveTo(.272*W-14*s,py+ph*.045+4*s); c.lineTo(.272*W-14*s,py+ph*.045);
      c.lineTo(.376*W+14*s,py+ph*.045); c.lineTo(.376*W+14*s,py+ph*.045+4*s); c.stroke();
      textSpaced(d,.324*W,py+ph*.05,F.barlow,7,inkDF,'EQUALIZER',0.05);
      // palanca CHORUS
      textSpaced(d,.612*W,topY,F.barlow,8,inkD,'CHORUS',0.02);
      ...
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      c.font=`italic 800 ${Math.round(13*s)}px ${F.barlow}`; c.fillStyle=rgb(242,243,246);
      c.fillText('JAZZ CHORUS-120H', .791*W, py+ph*.28); c.restore();
      const ry=py+ph*.70;
      // piloto rojo en su cuadradito
      rr(c,.652*W,ry-13*s,26*s,26*s,2.5*s); c.fillStyle=rgb(9,9,10); c.fill();
      c.strokeStyle='rgba(200,202,208,0.5)'; c.lineWidth=1*s; c.stroke();
      const plx=.652*W+13*s, ply=ry;
      c.save(); c.globalAlpha=0.5; c.beginPath(); c.arc(plx,ply,12*s,0,7);
      const gl=c.createRadialGradient(plx,ply,2*s,plx,ply,12*s); gl.addColorStop(0,'rgba(255,80,55,0.95)'); gl.addColorStop(1,'rgba(255,80,55,0)');
      c.fillStyle=gl; c.fill(); c.restore();
      c.beginPath(); c.arc(plx,ply,5*s,0,7);
      const pr=c.createRadialGradient(plx-1.2*s,ply-1.6*s,0.6*s,plx,ply,5*s);
      pr.addColorStop(0,rgb(255,150,120)); pr.addColorStop(.5,rgb(224,46,30)); pr.addColorStop(1,rgb(124,14,10));
      c.fillStyle=pr; c.fill();
      // bloque Ronald: monograma a la izquierda + nombre a la derecha (sin solape)
      rr(c,.678*W,ry-13*s,W*.062,26*s,2.5*s); c.fillStyle=rgb(9,9,10); c.fill();
      c.strokeStyle=rgb(160,162,168); c.lineWidth=1*s; c.stroke();
      monoR(.6815*W, ry-10*s, 0.20*s, 2.2*s, rgb(238,240,244));
      textSpaced(d,.678*W+W*.062*.63,ry,F.barlow,7,ink,'Ronald',0.01);
      // POWER
      batToggle(d,.874*W,ry,9*s,true);
      textSpaced(d,.902*W,ry-9*s,F.barlow,6.5,inkF,'ON',0.02);
      textSpaced(d,.902*W,ry+10*s,F.barlow,6.5,inkF,'OFF',0.02);
      textSpaced(d,.874*W,py+ph*.28,F.barlow,8,ink,'POWER',0.03);
      // ── tiras de ALUMINIO + rejilla hasta abajo (como el real) ──
      const gx=W*.062, gw=W*.876;
      const strip=(yy)=>{ rr(c,gx,yy,gw,H*.020,4*s);
        const sg2=c.createLinearGradient(0,yy,0,yy+H*.020);
        sg2.addColorStop(0,rgb(240,242,246)); sg2.addColorStop(.5,rgb(170,173,178)); sg2.addColorStop(1,rgb(216,219,224));
        c.fillStyle=sg2; c.fill(); c.strokeStyle=rgb(58,60,64); c.lineWidth=0.8*s; c.stroke(); };
      const gy=H*.400, gh=H*.435;
      strip(H*.362); strip(H*.848);
      rr(c,gx,gy-8*s,gw,gh+16*s,3*s); c.fillStyle=rgb(8,8,9); c.fill();
      rr(c,gx+3*s,gy,gw-6*s,gh,3*s); c.fillStyle=rgb(13,13,14); c.fill();
      c.save(); rr(c,gx+3*s,gy,gw-6*s,gh,3*s); c.clip();
      c.strokeStyle='rgba(214,202,164,0.32)'; c.lineWidth=1;
      for(let xx=gx;xx<gx+gw;xx+=6.5*s){ c.beginPath(); c.moveTo(xx,gy); c.lineTo(xx,gy+gh); c.stroke(); }
      for(let yy=gy;yy<gy+gh;yy+=6.5*s){ c.beginPath(); c.moveTo(gx,yy); c.lineTo(gx+gw,yy); c.stroke(); }
      c.strokeStyle='rgba(0,0,0,0.55)'; c.lineWidth=1.5*s;
      for(let xx=gx+3.2*s;xx<gx+gw;xx+=6.5*s){ c.beginPath(); c.moveTo(xx,gy); c.lineTo(xx,gy+gh); c.stroke(); }
      for(let yy=gy+3.2*s;yy<gy+gh;yy+=6.5*s){ c.beginPath(); c.moveTo(gx,yy); c.lineTo(gx+gw,yy); c.stroke(); }
      const sheen=c.createRadialGradient(gx+gw*.5,gy+gh*.45,gh*.3,gx+gw*.5,gy+gh*.5,gw*.55);
      sheen.addColorStop(0,'rgba(255,255,255,0.05)'); sheen.addColorStop(1,'rgba(0,0,0,0.30)');
      c.fillStyle=sheen; c.fillRect(gx,gy,gw,gh);
      c.restore();
      // placa Ronald (monograma de cinta + nombre, como la foto)
      const bpw=W*.125, bph=gh*.72, bpx=gx+gw*.030, bpy=gy+gh*.14;
      rr(c,bpx,bpy,bpw,bph,4*s); c.fillStyle=rgb(8,8,9); c.fill();
      rr(c,bpx,bpy,bpw,bph,4*s); c.strokeStyle=rgb(220,222,227); c.lineWidth=2*s; c.stroke();
      monoR(bpx+bpw*.20, bpy+bph*.08, bpw*.006, 7*s, rgb(240,242,246));
      c.save(); c.textAlign='center'; c.textBaseline='middle';
      c.font=`700 ${Math.round(19*s)}px ${F.barlow}`; c.fillStyle=rgb(240,242,246);
      c.fillText('Ronald', bpx+bpw/2, bpy+bph*.84); c.restore();
      // ── tachas + esquineras ──
      ...
      for(let i=0;i<16;i++){ const xx=W*(.075+.85*i/15); stud(xx,H*.045); stud(xx,H*.945); }
  ...
  const RB_AMP_FONT_SCALE = {
    ...
    marstenvs100:1.45, marstenjcm800:1.60, marstenjtm45:1.70, marstenbluesbreaker:1.60, marstenmajor:1.55, deezelvh4:1.45, raneyironheart:1.45,
    marstendsl15:1.60, marstenjvm410:1.60, marstensilverjubilee:1.62, aor50:1.38, jc90:1.15, engelfireball:1.45,
    polystoneminibrute:1.28, ronaldjc120:1.60, tw40:1.45, benderdeluxe:1.45, tw26:1.45, superdrive45:1.45,
  ...
  function drawSpec(canvas, spec, values) {
    ...
    (spec.switches || []).forEach(s => {
      ...
      else switchSquare(d, s.cx * d.W, s.cy * d.H, s.hs * d.W, on, s.dark, s.col);
    });
    (spec.sw3 || []).forEach(s => {
    ...
    (spec.sliders || []).forEach(sl => {
      ...
      hSlider(d, sl.x0 * d.W, sl.x1 * d.W, sl.y * d.H, v, sl.col);
    });
  ...
  function attach(canvas, stem, opts) {
    ...
    // instead of a registry key — use it directly in that case.
    const spec = (stem && typeof stem === 'object')
      ? stem
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  Custom user gear — serializable UI "layouts" + editable specs
  //
  //  A LAYOUT is the JSON-safe description of a custom gear's face, persisted
  //  with the gear (rig_builder_custom_gear.json → ui:). Shape:
  //    { type:'amp'|'cab'|'pedal'|'rack', title, w, h, body:[r,g,b],
  //      controls:[ {id, kind:'knob'|'slider'|'switch', label, cx, cy, r, hs,
  //                  color:[r,g,b]} ] }   (cx/cy/r/hs are fractions of w/h)
  //
  //  specFromLayout() turns a layout into a P-style spec that drawSpec()/attach()
  //  render — so custom gear rides the SAME draw + interaction path as the
  //  bundled pedals. NAM/IR gear (no plugin params) gets a default face too.
  // ══════════════════════════════════════════════════════════════════════════
  const _CL = (v) => v < 0 ? 0 : v > 255 ? 255 : v | 0;
  function hexToRgb(hex) {
    if (Array.isArray(hex)) return [_CL(hex[0]), _CL(hex[1]), _CL(hex[2])];
    const m = /^#?([0-9a-fA-F]{6})$/.exec((hex || '').trim());
    if (!m) return [120, 122, 128];
    const n = parseInt(m[1], 16);
    return [(n >> 16) & 255, (n >> 8) & 255, n & 255];
  }
  function rgbToHex(c) {
    const a = Array.isArray(c) ? c : hexToRgb(c);
    return '#' + [_CL(a[0]), _CL(a[1]), _CL(a[2])].map(v => v.toString(16).padStart(2, '0')).join('');
  }

  const RB_CTRL_KINDS = ['knob', 'slider', 'switch'];
  const _DEF_LABELS = {
    amp:   ['Gain', 'Bass', 'Mid', 'Treble', 'Presence', 'Master'],
    cab:   ['Level', 'Low', 'High'],
    pedal: ['Drive', 'Tone', 'Level'],
    rack:  ['Input', 'Mix', 'Output'],
  };
  const _DEF_BODY   = { amp: [58, 60, 66], cab: [42, 34, 28], pedal: [38, 120, 150], rack: [46, 48, 54] };
  const _DEF_ACCENT = { amp: [222, 214, 186], cab: [206, 180, 150], pedal: [232, 232, 226], rack: [120, 200, 150] };
  // Taller amp + rack faces so the knobs sit at a sensible size (they looked
  // oversized on the short defaults). Pedal/cab kept as-is.
  // Less-wide amp/cab/rack so they render at a size closer to the (tall) pedal
  // in the narrow editor/catalog panels instead of a short wide strip.
  const _DEF_DIMS   = { amp: [560, 440], cab: [540, 420], pedal: [300, 430], rack: [600, 380] };
  // panel rect [x, y, w, h] as fractions — where the controls are laid out.
  const _DEF_PANEL  = { amp: [0.07, 0.32, 0.86, 0.54], cab: [0.10, 0.60, 0.80, 0.26],
                        pedal: [0.16, 0.16, 0.68, 0.38], rack: [0.14, 0.34, 0.74, 0.40] };

  // ── smart-layout classification: group a VST's parameter by its NAME so the
  //    generated face orders + colours related controls together (a musically
  //    sensible drive → EQ → mod → time → mix → level flow). `big` groups get a
  //    slightly larger knob (the "hero" controls: gain, master/volume). ────────
  const _SMART_GROUPS = [
    { key: 'drive', ord: 0, big: true,  color: [214, 120, 90],
      rx: /gain|drive|dist|fuzz|overdrive|\bod\b|pre[\s-]?amp|crunch|\bsat|sustain/ },
    { key: 'eq',    ord: 1, big: false, color: [150, 170, 214],
      rx: /bass|\blow\b|\bmid|treble|\bhigh\b|presence|contour|tone|\beq\b|freq|\bhz\b|\bq\b|body|colou?r|character|shift/ },
    { key: 'mod',   ord: 2, big: false, color: [214, 182, 110],
      rx: /depth|feed\s?back|regen|resonance|\breso|width|intensity|\brate\b|speed|shape|sweep/ },
    { key: 'time',  ord: 3, big: false, color: [120, 194, 200],
      rx: /time|delay|\btempo|subdiv|pre[\s-]?delay|decay|\bsize\b|room|hall|space/ },
    { key: 'mix',   ord: 4, big: false, color: [176, 146, 214],
      rx: /mix|blend|\bwet\b|\bdry\b|balance|amount/ },
    { key: 'level', ord: 5, big: true,  color: [130, 200, 150],
      rx: /level|volume|master|output|\bout\b|\bvol\b|make[\s-]?up|trim/ },
  ];
  const _SMART_MISC = { key: 'misc', ord: 6, big: false, color: [150, 152, 158] };
  // Classic on/off amp toggles that read better as a switch than a knob.
  const _SMART_SWITCH_RX = /\b(bright|standby|power|deep|\bfat\b|thick|boost|voicing|active|fizz|scoop)\b/;

  function _smartClassify(name) {
    const n = (name || '').toLowerCase();
    for (const g of _SMART_GROUPS) if (g.rx.test(n)) return g;
    return _SMART_MISC;
  }

  function _normType(t) {
    t = (t || 'pedal').toLowerCase();
    return ({ amps: 'amp', cabs: 'cab', pedals: 'pedal', racks: 'rack' })[t] || t;
  }

  // Build a fresh default layout for a gear type from a list of control labels
  // (falls back to sensible per-type defaults). All controls start as knobs in
  // the type's accent colour; the editor lets the user change kind/colour/label.
  function defaultFace(type, labels, opts) {
    opts = opts || {};
    type = _normType(type);
    if (!_DEF_DIMS[type]) type = 'pedal';
    // A cab is a speaker box — no knobs by default (it's an IR / cabinet). Only
    // add controls if labels were explicitly passed (e.g. a cab VST with params).
    if (type === 'cab' && !(labels && labels.length)) {
      const [cw, ch] = _DEF_DIMS.cab;
      return { type, title: opts.title || '', template: _DEF_TEMPLATE.cab, w: cw, h: ch, body: (_DEF_BODY.cab || [42, 34, 28]).slice(), controls: [] };
    }
    labels = (labels && labels.length) ? labels.slice(0, 12) : (_DEF_LABELS[type] || ['Gain', 'Tone', 'Level']);
    const [w, h] = _DEF_DIMS[type];
    const [px, py, pw, ph] = _DEF_PANEL[type];
    const n = labels.length;
    const maxCols = type === 'pedal' ? 3 : (type === 'rack' ? 6 : 4);
    const cols = Math.min(n, maxCols), rows = Math.ceil(n / cols);
    const accent = (_DEF_ACCENT[type] || [86, 150, 210]).slice();
    // Size the knob from the CELL in pixels (fits both width and height, leaving
    // room for the label below), then express it as a fraction of width. The
    // hard cap stops a wide face (amp/rack) from getting oversized knobs.
    const cellWpx = (pw * w) / cols, cellHpx = (ph * h) / rows;
    const rPx = Math.min(cellWpx * 0.28, cellHpx * 0.30, 40);
    const r = rPx / w;
    const hs = Math.min(rPx * 0.7, 22) / w;
    const controls = labels.map((lb, i) => {
      const cc = i % cols, rw = Math.floor(i / cols);
      // Nudge each knob toward the top of its cell so the label fits below it.
      const cellTopBias = rows > 1 ? 0.40 : 0.42;
      return {
        id: i, kind: 'knob', label: lb,
        cx: px + pw * ((cc + 0.5) / cols),
        cy: py + ph * ((rw + cellTopBias) / rows),
        r, hs, color: accent.slice(),
      };
    });
    return { type, title: opts.title || '', template: _DEF_TEMPLATE[type] || 'box', w, h, body: (_DEF_BODY[type] || [40, 42, 48]).slice(), controls };
  }

  // Smart layout from a plugin's REAL parameters: order + colour controls by
  // their function (drive → EQ → mod → time → mix → level), make the hero
  // controls (gain, master) a touch bigger, and honour each param's step count —
  // a 2-state param becomes a switch, a >2-step "mode" param (e.g. a Pultec
  // band-select) becomes a STEPPED selector knob (a drag/click snaps to the next
  // position instead of sweeping continuously). `params` = the getParameters()
  // array (name/label + optional numSteps), already filtered to sound params.
  function smartLayout(type, params, opts) {
    opts = opts || {};
    type = _normType(type);
    if (!_DEF_DIMS[type]) type = 'pedal';
    const [w, h] = _DEF_DIMS[type];
    const [px, py, pw, ph] = _DEF_PANEL[type];
    const items = (params || []).slice(0, 12).map((p, i) => {
      const name = String((p && (p.name || p.label)) || ('P' + (i + 1))).trim();
      const g = _smartClassify(name);
      const steps = (p && (p.numSteps || p.stepCount)) || 0;
      const ticks = (p && Array.isArray(p.ticks)) ? p.ticks : null;
      let kind = 'knob', select = 0;
      if (steps === 2 || _SMART_SWITCH_RX.test(name.toLowerCase())) kind = 'switch';
      else if (steps > 2 && steps <= 16) { kind = 'knob'; select = steps; }   // stepped "mode" knob
      // _i is this param's LOGICAL index (position in the filtered param list) —
      // preserved as `param` so the live editor can drive the right plugin param
      // even after the smart sort reorders the controls.
      return { name: name.slice(0, 14), color: g.color.slice(), big: !!g.big, ord: g.ord, kind, select, ticks, param: i, _i: i };
    });
    // Stable sort into the musical group order.
    items.sort((a, b) => (a.ord - b.ord) || (a._i - b._i));
    const n = items.length || 1;
    const maxCols = type === 'pedal' ? 3 : (type === 'rack' ? 6 : 4);
    const cols = Math.min(n, maxCols) || 1, rows = Math.ceil(n / cols) || 1;
    const cellWpx = (pw * w) / cols, cellHpx = (ph * h) / rows;
    const baseR = Math.min(cellWpx * 0.27, cellHpx * 0.28, 38) / w;
    const controls = items.map((it, idx) => {
      const cc = idx % cols, rw = Math.floor(idx / cols);
      const r = it.kind === 'switch' ? baseR * 0.6 : (it.big ? baseR * 1.16 : baseR);
      const cellTopBias = rows > 1 ? 0.40 : 0.42;
      const ctrl = {
        id: idx, kind: it.kind, label: it.name,
        cx: px + pw * ((cc + 0.5) / cols),
        cy: py + ph * ((rw + cellTopBias) / rows),
        r, hs: Math.min(r * 0.6, 0.03), color: it.color,
      };
      if (it.select > 2) ctrl.select = it.select;   // carries into the spec knob as a stepper
      if (it.ticks && it.ticks.length >= 2) ctrl.ticks = it.ticks.slice(0, 16);   // value labels around the knob
      if (typeof it.param === 'number') ctrl.param = it.param;   // → real plugin param (logical id)
      return ctrl;
    });
    return { type, title: opts.title || '', template: _DEF_TEMPLATE[type] || 'box', w, h, body: (_DEF_BODY[type] || [40, 42, 48]).slice(), controls };
  }

  // Coerce any stored/edited layout into a well-formed one (colours → [r,g,b],
  // ids present, kinds valid). Tolerant of hand-edited / partial JSON.
  function normalizeLayout(layout) {
    const type = _normType(layout && layout.type);
    const dims = _DEF_DIMS[type] || _DEF_DIMS.pedal;
    const ctrls = ((layout && layout.controls) || []).map((c, i) => ({
      id: (c.id != null ? c.id : i),
      kind: RB_CTRL_KINDS.indexOf(c.kind) >= 0 ? c.kind : 'knob',
      label: c.label || '',
      cx: (typeof c.cx === 'number') ? c.cx : 0.5,
      cy: (typeof c.cy === 'number') ? c.cy : 0.5,
      r: (typeof c.r === 'number') ? c.r : 0.06,
      hs: (typeof c.hs === 'number') ? c.hs : 0.032,
      color: hexToRgb(c.color || _DEF_ACCENT[type] || [120, 122, 128]),
      // Optional discrete-mode metadata (stepped selector knobs + the value
      // labels drawn around them, e.g. a Pultec's 200/300/500/700/1000).
      ...(c.select > 2 ? { select: c.select } : {}),
      ...(Array.isArray(c.ticks) && c.ticks.length ? { ticks: c.ticks.slice(0, 16) } : {}),
      ...(typeof c.param === 'number' ? { param: c.param } : {}),
    }));
    return {
      type, title: (layout && layout.title) || '',
      template: (layout && layout.template) || _DEF_TEMPLATE[type] || 'box',
      w: (layout && layout.w) || dims[0], h: (layout && layout.h) || dims[1],
      body: hexToRgb((layout && layout.body) || _DEF_BODY[type] || [40, 42, 48]),
      controls: ctrls,
    };
  }

  // ── per-type faceplates (drawn under the controls) ──────────────────────────
  function _brushed(d, b) {
    const { ctx: c, W, H } = d;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(_CL(b[0] + 20), _CL(b[1] + 20), _CL(b[2] + 20)));
    g.addColorStop(1, rgb(_CL(b[0] - 16), _CL(b[1] - 16), _CL(b[2] - 16)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    c.strokeStyle = 'rgba(255,255,255,0.028)'; c.lineWidth = 1;
    for (let y = 0; y < H; y += 3) { c.beginPath(); c.moveTo(0, y); c.lineTo(W, y); c.stroke(); }
    c.strokeStyle = rgb(18, 18, 22); c.lineWidth = 3; c.strokeRect(2, 2, W - 4, H - 4);
  }
  function faceAmp(d, L) {
    const { ctx: c, W, H } = d; _brushed(d, L.body);
    rr(c, W * 0.06, H * 0.06, W * 0.88, H * 0.17, 7); c.fillStyle = rgb(14, 14, 16); c.fill();
    rr(c, W * 0.06, H * 0.06, W * 0.88, H * 0.17, 7); c.strokeStyle = rgb(6, 6, 8); c.lineWidth = 2; c.stroke();
    textC(d, W * 0.5, H * 0.148, F.bebas, 30, rgb(228, 229, 233), (L.title || 'AMP').toUpperCase());
    rr(c, W * 0.05, H * 0.30, W * 0.90, H * 0.60, 8); c.fillStyle = rgb(24, 24, 28); c.fill();
    rr(c, W * 0.05, H * 0.30, W * 0.90, H * 0.60, 8); c.strokeStyle = rgb(8, 8, 10); c.lineWidth = 2; c.stroke();
  }
  function faceCab(d, L) {
    const { ctx: c, W, H } = d;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(_CL(L.body[0] + 14), _CL(L.body[1] + 12), _CL(L.body[2] + 10)));
    g.addColorStop(1, rgb(_CL(L.body[0] - 12), _CL(L.body[1] - 12), _CL(L.body[2] - 12)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    // grille cloth (dot weave) in the upper region
    rr(c, W * 0.06, H * 0.06, W * 0.88, H * 0.46, 8); c.fillStyle = rgb(20, 18, 16); c.fill();
    c.save(); rr(c, W * 0.06, H * 0.06, W * 0.88, H * 0.46, 8); c.clip();
    c.fillStyle = 'rgba(210,200,180,0.10)';
    for (let y = H * 0.08; y < H * 0.52; y += 9) for (let x = W * 0.08; x < W * 0.92; x += 9) {
      c.beginPath(); c.arc(x, y, 1.5, 0, 7); c.fill();
    }
    c.restore();
    rr(c, W * 0.06, H * 0.06, W * 0.88, H * 0.46, 8); c.strokeStyle = rgb(10, 8, 6); c.lineWidth = 2; c.stroke();
    textC(d, W * 0.5, H * 0.565, F.bebas, 22, rgb(224, 214, 196), (L.title || 'CAB').toUpperCase());
    rr(c, W * 0.08, H * 0.60, W * 0.84, H * 0.30, 6); c.fillStyle = rgb(26, 22, 18); c.fill();
  }
  function facePedal(d, L) {
    const b = L.body || [40, 120, 150];
    box(d, b[0], b[1], b[2], false);   // no screws
    textC(d, d.W * 0.5, d.H * 0.085, F.bebas, 22, rgb(245, 245, 240), (L.title || 'PEDAL').toUpperCase());
  }
  function faceRack(d, L) {
    const { ctx: c, W, H } = d, b = L.body;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(_CL(b[0] + 16), _CL(b[1] + 16), _CL(b[2] + 16)));
    g.addColorStop(1, rgb(_CL(b[0] - 14), _CL(b[1] - 14), _CL(b[2] - 14)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    // rack ears
    c.fillStyle = rgb(_CL(b[0] - 22), _CL(b[1] - 22), _CL(b[2] - 22));
    c.fillRect(0, 0, W * 0.055, H); c.fillRect(W * 0.945, 0, W * 0.055, H);
    c.strokeStyle = rgb(16, 16, 20); c.lineWidth = 2; c.strokeRect(2, 2, W - 4, H - 4);
    // green LCD nameplate
    rr(c, W * 0.09, H * 0.14, W * 0.30, H * 0.34, 4); c.fillStyle = rgb(12, 30, 16); c.fill();
    rr(c, W * 0.09, H * 0.14, W * 0.30, H * 0.34, 4); c.strokeStyle = rgb(40, 90, 50); c.lineWidth = 1.5; c.stroke();
    textC(d, W * 0.24, H * 0.31, F.barlow, 20, rgb(120, 240, 150), (L.title || 'RACK').toUpperCase());
  }
  // "Flat + screws" template — a Pultec MEQ5-style faceplate: a single solid
  // colour with a thin bevel and four corner pallet screws. Works for any type.
  function faceFlat(d, L) {
    const { ctx: c, W, H } = d, b = L.body;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(_CL(b[0] + 16), _CL(b[1] + 16), _CL(b[2] + 16)));
    g.addColorStop(1, rgb(_CL(b[0] - 14), _CL(b[1] - 14), _CL(b[2] - 14)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    c.strokeStyle = rgb(_CL(b[0] - 42), _CL(b[1] - 42), _CL(b[2] - 42)); c.lineWidth = 3;
    c.strokeRect(2.5, 2.5, W - 5, H - 5);
    const m = Math.min(W, H) * 0.06;                       // corner pallet screws
    [[m, m], [W - m, m], [m, H - m], [W - m, H - m]].forEach(p => screw(d, p[0], p[1]));
    if (L.title) textC(d, W * 0.5, H * 0.11, F.bebas, 22, rgb(238, 238, 233), L.title.toUpperCase());
  }
  // ── PEDAL templates (reuse the existing stompbox body-makers) ──────────────
  function faceChief(d, L) {   // Boss-compact stompbox
    const b = L.body || [60, 120, 150];
    chiefBody(d, b[0], b[1], b[2]);
    if (L.title) textC(d, d.W * 0.5, d.H * 0.045, F.bebas, 15, rgb(245, 245, 240), L.title.toUpperCase());
  }
  function faceIbanez(d, L) {   // Ibanez Tonelok-style silver enclosure (fixed finish)
    ibanezBody(d);
    if (L.title) textC(d, d.W * 0.5, d.H * 0.045, F.bebas, 14, rgb(44, 46, 52), L.title.toUpperCase());
  }
  function faceFoog(d, L) {     // Moogerfooger-style granite + wood sides (fixed finish)
    foogBody(d);
    if (L.title) textC(d, d.W * 0.5, d.H * 0.05, F.bebas, 14, rgb(232, 227, 216), L.title.toUpperCase());
  }
  function facePlainBox(d, L) { // plain enclosure, no screws (the pedal default)
    const b = L.body || [40, 120, 150];
    box(d, b[0], b[1], b[2], false);
    if (L.title) textC(d, d.W * 0.5, d.H * 0.075, F.bebas, 20, rgb(245, 245, 240), L.title.toUpperCase());
  }
  // ── AMP templates ──────────────────────────────────────────────────────────
  function faceAmpTolex(d, L) { // tolex-covered head with a gold control strip
    const { ctx: c, W, H } = d, b = L.body;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(_CL(b[0] + 8), _CL(b[1] + 8), _CL(b[2] + 8)));
    g.addColorStop(1, rgb(_CL(b[0] - 18), _CL(b[1] - 18), _CL(b[2] - 18)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    c.strokeStyle = 'rgba(0,0,0,0.10)'; c.lineWidth = 1;               // tolex weave
    for (let y = 0; y < H; y += 5) { c.beginPath(); c.moveTo(0, y); c.lineTo(W, y); c.stroke(); }
    c.strokeStyle = 'rgba(240,238,230,0.55)'; c.lineWidth = 2; c.strokeRect(W * 0.03, H * 0.05, W * 0.94, H * 0.90);
    rr(c, W * 0.05, H * 0.32, W * 0.90, H * 0.52, 6);                  // gold control strip
    const sg = c.createLinearGradient(0, H * 0.32, 0, H * 0.84);
    sg.addColorStop(0, rgb(64, 56, 34)); sg.addColorStop(0.5, rgb(154, 132, 72)); sg.addColorStop(1, rgb(70, 60, 36));
    c.fillStyle = sg; c.fill();
    rr(c, W * 0.05, H * 0.32, W * 0.90, H * 0.52, 6); c.strokeStyle = rgb(30, 26, 14); c.lineWidth = 2; c.stroke();
    textC(d, W * 0.5, H * 0.19, F.bebas, 26, rgb(238, 232, 214), (L.title || 'AMP').toUpperCase());
  }
  function faceRackStd(d, L) {  // 1U studio faceplate (as the Studio Delay etc. use)
    rackFace(d, { accent: L.body || [90, 110, 150], title: (L.title || 'RACK').toUpperCase() }, []);
  }

  // ── Generic colour-driven faceplates (respect the Body colour) ──────────────
  function _tt(d, L, col, cyFrac, px) {
    if (L.title) textC(d, d.W * 0.5, d.H * cyFrac, F.bebas, px || 22, col, L.title.toUpperCase());
  }
  function _frame(d, col, lw) {
    const { ctx: c, W, H } = d; c.strokeStyle = col; c.lineWidth = lw || 3;
    c.strokeRect((lw || 3) / 2, (lw || 3) / 2, W - (lw || 3), H - (lw || 3));
  }
  function faceBrushed(d, L) { _brushed(d, L.body); _tt(d, L, rgb(232, 233, 237), 0.09); }
  function faceMatte(d, L) {
    const { ctx: c, W, H } = d, b = L.body;
    c.fillStyle = rgb(_CL(b[0]), _CL(b[1]), _CL(b[2])); c.fillRect(0, 0, W, H);
    rr(c, W * 0.04, H * 0.04, W * 0.92, H * 0.92, 14); c.strokeStyle = 'rgba(255,255,255,0.10)'; c.lineWidth = 2; c.stroke();
    rr(c, W * 0.04, H * 0.04, W * 0.92, H * 0.92, 14); c.strokeStyle = 'rgba(0,0,0,0.35)'; c.lineWidth = 1; c.stroke();
    _tt(d, L, rgb(240, 240, 236), 0.10);
  }
  function faceCarbon(d, L) {
    // Real carbon-fibre twill: dark charcoal base + two low-contrast diagonal
    // thread sets (NOT a white/grey checkerboard, which read like a transparency
    // pattern). Body colour tints the base only slightly.
    const { ctx: c, W, H, s } = d, b = L.body;
    const g = c.createLinearGradient(0, 0, W, H);
    g.addColorStop(0, rgb(_CL(20 + b[0] * 0.10), _CL(20 + b[1] * 0.10), _CL(22 + b[2] * 0.10)));
    g.addColorStop(0.5, rgb(_CL(12 + b[0] * 0.08), _CL(12 + b[1] * 0.08), _CL(14 + b[2] * 0.08)));
    g.addColorStop(1, rgb(_CL(24 + b[0] * 0.10), _CL(24 + b[1] * 0.10), _CL(26 + b[2] * 0.10)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    c.save(); c.beginPath(); c.rect(0, 0, W, H); c.clip();
    const step = 6 * s;
    c.lineWidth = 2 * s; c.strokeStyle = 'rgba(150,152,158,0.10)';          // ↘ threads (light)
    for (let x = -H; x < W; x += step) { c.beginPath(); c.moveTo(x, 0); c.lineTo(x + H, H); c.stroke(); }
    c.lineWidth = 2 * s; c.strokeStyle = 'rgba(0,0,0,0.28)';                 // ↗ threads (dark)
    for (let x = 0; x < W + H; x += step) { c.beginPath(); c.moveTo(x, 0); c.lineTo(x - H, H); c.stroke(); }
    c.lineWidth = 0.8 * s; c.strokeStyle = 'rgba(190,192,198,0.06)';        // sheen highlight
    for (let x = -H; x < W; x += step * 2) { c.beginPath(); c.moveTo(x + 1.5 * s, 0); c.lineTo(x + 1.5 * s + H, H); c.stroke(); }
    c.restore(); _frame(d, rgb(16, 16, 18), 3); _tt(d, L, rgb(226, 228, 234), 0.09);
  }
  function faceWood(d, L) {
    const { ctx: c, W, H } = d, b = L.body;
    const base = [Math.max(60, b[0]), Math.max(38, b[1] * 0.7), Math.max(22, b[2] * 0.5)];
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(_CL(base[0] + 16), _CL(base[1] + 12), _CL(base[2] + 8)));
    g.addColorStop(1, rgb(_CL(base[0] - 18), _CL(base[1] - 14), _CL(base[2] - 10)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    c.strokeStyle = 'rgba(60,36,18,0.32)'; c.lineWidth = 1;
    for (let i = 0; i < 14; i++) {
      const y = H * (i + 0.5) / 14; c.beginPath(); c.moveTo(0, y);
      for (let x = 0; x <= W; x += W / 12) c.lineTo(x, y + Math.sin((x / W) * 6 + i) * 3);
      c.stroke();
    }
    _frame(d, rgb(40, 26, 14), 4); _tt(d, L, rgb(245, 235, 215), 0.10);
  }
  function faceChrome(d, L) {
    const { ctx: c, W, H } = d;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(214, 218, 224)); g.addColorStop(0.45, rgb(150, 156, 164));
    g.addColorStop(0.5, rgb(120, 126, 134)); g.addColorStop(0.55, rgb(172, 178, 186)); g.addColorStop(1, rgb(120, 124, 130));
    c.fillStyle = g; c.fillRect(0, 0, W, H); _frame(d, rgb(70, 74, 80), 3); _tt(d, L, rgb(40, 42, 48), 0.10);
  }
  function faceVintage(d, L) {
    const { ctx: c, W, H } = d, b = L.body;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(236, 226, 198)); g.addColorStop(1, rgb(210, 198, 168));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    c.fillStyle = rgb(_CL(b[0]), _CL(b[1]), _CL(b[2])); c.fillRect(0, H * 0.03, W, H * 0.05);
    _frame(d, rgb(150, 132, 96), 3); _tt(d, L, rgb(70, 54, 32), 0.17);
  }
  function faceMarsten(d, L) { marstenGoldHead(d, (L.title || '').toUpperCase()); }   // gold amp head (fixed finish)
  function faceCabVintage(d, L) {
    const { ctx: c, W, H } = d, b = L.body;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(_CL(b[0] + 10), _CL(b[1] + 8), _CL(b[2] + 6)));
    g.addColorStop(1, rgb(_CL(b[0] - 14), _CL(b[1] - 12), _CL(b[2] - 10)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    rr(c, W * 0.06, H * 0.06, W * 0.88, H * 0.62, 8); c.fillStyle = rgb(24, 20, 16); c.fill();
    c.save(); rr(c, W * 0.06, H * 0.06, W * 0.88, H * 0.62, 8); c.clip();
    c.strokeStyle = 'rgba(200,186,160,0.10)'; c.lineWidth = 1.4;
    for (let x = -H; x < W; x += 7) { c.beginPath(); c.moveTo(x, 0); c.lineTo(x + H, H); c.stroke(); }
    for (let x = -H; x < W; x += 7) { c.beginPath(); c.moveTo(x, H); c.lineTo(x + H, 0); c.stroke(); }
    c.restore();
    rr(c, W * 0.06, H * 0.06, W * 0.88, H * 0.62, 8); c.strokeStyle = rgb(12, 10, 8); c.lineWidth = 2; c.stroke();
    _tt(d, L, rgb(224, 214, 196), 0.74, 20);
  }

  // ── distinct AMP archetypes (control panel sits in the knob band; the top /
  //    body / framing differ so they read as different amps, not just colours) ──
  function _ampPanel(d, grad, piping) {   // panel across the knob band + optional white piping
    const { ctx: c, W, H } = d;
    rr(c, W * 0.045, H * 0.26, W * 0.91, H * 0.66, 7); c.fillStyle = grad; c.fill();
    rr(c, W * 0.045, H * 0.26, W * 0.91, H * 0.66, 7); c.strokeStyle = 'rgba(0,0,0,0.5)'; c.lineWidth = 2; c.stroke();
    if (piping) { c.strokeStyle = 'rgba(232,228,214,0.55)'; c.lineWidth = 2; c.strokeRect(W * 0.025, H * 0.04, W * 0.95, H * 0.92); }
  }
  function faceBlackface(d, L) {   // black tolex + brushed SILVER control panel
    const { ctx: c, W, H } = d;
    c.fillStyle = rgb(17, 17, 19); c.fillRect(0, 0, W, H);
    c.strokeStyle = 'rgba(255,255,255,0.02)'; c.lineWidth = 1;
    for (let y = 0; y < H; y += 4) { c.beginPath(); c.moveTo(0, y); c.lineTo(W, y); c.stroke(); }
    const g = c.createLinearGradient(0, H * 0.26, 0, H * 0.92);
    g.addColorStop(0, rgb(210, 214, 220)); g.addColorStop(0.5, rgb(168, 172, 180)); g.addColorStop(1, rgb(144, 148, 156));
    _ampPanel(d, g, true);
    c.save(); rr(c, W * 0.045, H * 0.26, W * 0.91, H * 0.66, 7); c.clip();     // brushed lines
    c.strokeStyle = 'rgba(255,255,255,0.08)'; c.lineWidth = 1;
    for (let x = 0; x < W; x += 2) { c.beginPath(); c.moveTo(x, H * 0.26); c.lineTo(x, H * 0.92); c.stroke(); }
    c.restore();
    _tt(d, L, rgb(232, 232, 228), 0.155, 22);
  }
  function facePlexi(d, L) {   // black + brass/gold plexi control panel + piping
    const { ctx: c, W, H } = d;
    c.fillStyle = rgb(14, 13, 15); c.fillRect(0, 0, W, H);
    const g = c.createLinearGradient(0, H * 0.26, 0, H * 0.92);
    g.addColorStop(0, rgb(190, 158, 78)); g.addColorStop(0.5, rgb(158, 128, 60)); g.addColorStop(1, rgb(120, 96, 44));
    _ampPanel(d, g, true);
    rr(c, W * 0.30, H * 0.05, W * 0.40, H * 0.15, 5); c.fillStyle = rgb(10, 10, 12); c.fill();   // black logo plate
    rr(c, W * 0.30, H * 0.05, W * 0.40, H * 0.15, 5); c.strokeStyle = rgb(150, 124, 60); c.lineWidth = 1.5; c.stroke();
    _tt(d, L, rgb(214, 182, 96), 0.125, 20);
  }
  function _tweedWeave(d) {
    const { ctx: c, W, H, s } = d;
    const bg = c.createLinearGradient(0, 0, 0, H); bg.addColorStop(0, rgb(206, 170, 98)); bg.addColorStop(0.5, rgb(190, 154, 86)); bg.addColorStop(1, rgb(168, 132, 70));
    c.fillStyle = bg; c.fillRect(0, 0, W, H);
    c.save(); c.beginPath(); c.rect(0, 0, W, H); c.clip();
  ...
  function faceTweed(d, L) {   // lacquered tweed cloth + dark chrome control chassis + handle
    const { ctx: c, W, H, s } = d;
    _tweedWeave(d);
    // leather strap handle (top centre)
    const hx0 = W * 0.38, hx1 = W * 0.62, hcy = H * 0.10, hth = H * 0.045;
    const lg = c.createLinearGradient(0, hcy - hth, 0, hcy + hth); lg.addColorStop(0, rgb(108, 66, 38)); lg.addColorStop(1, rgb(56, 30, 16));
    ...
    const g = c.createLinearGradient(0, H * 0.26, 0, H * 0.92);
    g.addColorStop(0, rgb(44, 44, 48)); g.addColorStop(0.5, rgb(24, 24, 27)); g.addColorStop(1, rgb(14, 14, 16));
    _ampPanel(d, g, false);
    _tt(d, L, rgb(228, 224, 214), 0.205, 20);
  }

  // ── more CAB styles (cabs have no default knobs — pure speaker-box faces) ────
  function faceCab4x12(d, L) {   // 4x12: grille with four visible speaker cones
    const { ctx: c, W, H } = d, b = L.body;
    const g = c.createLinearGradient(0, 0, 0, H);
    g.addColorStop(0, rgb(_CL(b[0] + 8), _CL(b[1] + 6), _CL(b[2] + 4))); g.addColorStop(1, rgb(_CL(b[0] - 16), _CL(b[1] - 14), _CL(b[2] - 12)));
    c.fillStyle = g; c.fillRect(0, 0, W, H);
    rr(c, W * 0.05, H * 0.05, W * 0.90, H * 0.90, 10); c.fillStyle = rgb(20, 18, 16); c.fill();
    const cone = (cx, cy, r) => {
      c.beginPath(); c.arc(cx, cy, r, 0, 7); c.fillStyle = rgb(30, 28, 26); c.fill();
      c.strokeStyle = rgb(70, 66, 60); c.lineWidth = 2; c.stroke();
      c.beginPath(); c.arc(cx, cy, r * 0.62, 0, 7); const rg = c.createRadialGradient(cx - r * 0.2, cy - r * 0.2, r * 0.1, cx, cy, r * 0.7);
      rg.addColorStop(0, rgb(60, 56, 52)); rg.addColorStop(1, rgb(26, 24, 22)); c.fillStyle = rg; c.fill();
      c.beginPath(); c.arc(cx, cy, r * 0.20, 0, 7); c.fillStyle = rgb(18, 16, 15); c.fill();   // dust cap
    };
    const r = Math.min(W, H) * 0.17;
    cone(W * 0.30, H * 0.30, r); cone(W * 0.70, H * 0.30, r); cone(W * 0.30, H * 0.70, r); cone(W * 0.70, H * 0.70, r);
    rr(c, W * 0.05, H * 0.05, W * 0.90, H * 0.90, 10); c.strokeStyle = rgb(10, 9, 8); c.lineWidth = 3; c.stroke();
    if (L.title) { rr(c, W * 0.36, H * 0.905, W * 0.28, H * 0.055, 3); c.fillStyle = rgb(12, 11, 10); c.fill(); _tt(d, L, rgb(200, 200, 196), 0.933, 12); }
  }
  function faceCabMarsten(d, L) {   // grille + gold logo bar
    faceCab(d, { ...L, title: '' });
    const { ctx: c, W, H } = d;
    rr(c, W * 0.30, H * 0.905, W * 0.40, H * 0.06, 4); const g = c.createLinearGradient(0, H * 0.905, 0, H * 0.965);
    g.addColorStop(0, rgb(224, 198, 130)); g.addColorStop(1, rgb(150, 124, 68)); c.fillStyle = g; c.fill();
    rr(c, W * 0.30, H * 0.905, W * 0.40, H * 0.06, 4); c.strokeStyle = rgb(90, 72, 34); c.lineWidth = 1.5; c.stroke();
    if (L.title) textC(d, W * 0.5, H * 0.935, F.bebas, 15, rgb(40, 30, 12), L.title.toUpperCase());
  }
  function faceCabBender(d, L) {   // silver sparkle grille + script logo
    const { ctx: c, W, H } = d;
    c.fillStyle = rgb(28, 26, 24); c.fillRect(0, 0, W, H);
    rr(c, W * 0.05, H * 0.05, W * 0.90, H * 0.90, 8); c.fillStyle = rgb(140, 140, 146); c.fill();
    c.save(); rr(c, W * 0.05, H * 0.05, W * 0.90, H * 0.90, 8); c.clip();
    // sparkle
    for (let i = 0; i < 380; i++) {
      const x = (i * 97) % W, y = (i * 53) % H, v = (i * 29) % 3;
      c.fillStyle = v === 0 ? 'rgba(255,255,255,0.20)' : v === 1 ? 'rgba(0,0,0,0.16)' : 'rgba(210,214,220,0.14)';
      c.fillRect(x, y, 2, 2);
    }
    c.strokeStyle = 'rgba(0,0,0,0.10)'; c.lineWidth = 1;
    for (let x = -H; x < W; x += 8) { c.beginPath(); c.moveTo(x, 0); c.lineTo(x + H, H); c.stroke(); }
    c.restore();
    rr(c, W * 0.05, H * 0.05, W * 0.90, H * 0.90, 8); c.strokeStyle = rgb(60, 60, 64); c.lineWidth = 3; c.stroke();
    if (L.title) { setFont(d, F.ink, 26); c.fillStyle = rgb(40, 40, 46); c.textAlign = 'center'; c.textBaseline = 'middle'; c.fillText(L.title, W * 0.5, H * 0.5); }
  }
  function faceCabBox(d, L) {   // closed-back wood cab (no grille) + badge + screws
    faceWood(d, { ...L, title: '' });
    const { ctx: c, W, H } = d;
    rr(c, W * 0.34, H * 0.42, W * 0.32, H * 0.16, 5); c.fillStyle = rgb(20, 18, 16); c.fill();
    rr(c, W * 0.34, H * 0.42, W * 0.32, H * 0.16, 5); c.strokeStyle = rgb(150, 120, 70); c.lineWidth = 1.5; c.stroke();
    if (L.title) textC(d, W * 0.5, H * 0.50, F.bebas, 18, rgb(210, 186, 130), L.title.toUpperCase());
    const m = Math.min(W, H) * 0.06;
    [[m, m], [W - m, m], [m, H - m], [W - m, H - m]].forEach(p => screw(d, p[0], p[1]));
  }

  // Background styles, scoped by gear type. Each type's PRIMARY style (used when
  // no template is set / for legacy 'default') is `_DEF_TEMPLATE`.
  const _TEMPLATES = {
    // amp — structurally distinct archetypes first
    ampstd:    { label: 'Amp panel',      types: ['amp'],  draw: faceAmp },
    blackface: { label: 'Blackface',      types: ['amp'],  draw: faceBlackface },
    plexi:     { label: 'Plexi (gold)',   types: ['amp'],  draw: facePlexi },
    tweed:     { label: 'Tweed combo',    types: ['amp'],  draw: faceTweed },
    tolex:     { label: 'Tolex head',     types: ['amp'],  draw: faceAmpTolex },
    marsten:   { label: 'Amp head (gold)', types: ['amp'], draw: faceMarsten },
    // rack
    panel:   { label: 'LCD panel',      types: ['rack'], draw: faceRack },
    rackstd: { label: 'Studio rack',    types: ['rack'], draw: faceRackStd },
    // pedal
    box:     { label: 'Plain box',      types: ['pedal'], draw: facePlainBox },
    chief:   { label: 'Chief (Boss)',   types: ['pedal'], draw: faceChief },
    ibanez:  { label: 'Ibanez',         types: ['pedal'], draw: faceIbanez },
    foog:    { label: 'Moog / Foog',    types: ['pedal'], draw: faceFoog },
    // cab — pure speaker-box faces (NO flat, NO default knobs)
    grille:  { label: 'Grille',         types: ['cab'],  draw: faceCab },
    grille2: { label: 'Vintage grille', types: ['cab'],  draw: faceCabVintage },
    fourx12: { label: '4x12 speakers',  types: ['cab'],  draw: faceCab4x12 },
    cabmars: { label: 'Marsten cab',    types: ['cab'],  draw: faceCabMarsten },
    cabbend: { label: 'Bender sparkle', types: ['cab'],  draw: faceCabBender },
    cabbox:  { label: 'Closed box',     types: ['cab'],  draw: faceCabBox },
    // shared finishes (materials) — kept lean on amps so they don't all look alike
    brushed: { label: 'Brushed metal',  types: ['rack'], draw: faceBrushed },
    matte:   { label: 'Matte',          types: ['amp', 'pedal', 'rack'], draw: faceMatte },
    carbon:  { label: 'Carbon fibre',   types: ['amp', 'pedal', 'rack'], draw: faceCarbon },
    wood:    { label: 'Wood',           types: ['amp', 'pedal', 'rack', 'cab'], draw: faceWood },
    chrome:  { label: 'Chrome',         types: ['pedal', 'rack'], draw: faceChrome },
    vintage: { label: 'Vintage cream',  types: ['pedal'], draw: faceVintage },
    flat:    { label: 'Flat + screws',  types: ['pedal', 'rack'], draw: faceFlat },
  };
  const _TEMPLATE_ORDER = [
    'ampstd', 'blackface', 'plexi', 'tweed', 'tolex', 'marsten',
    'panel', 'rackstd',
    'box', 'chief', 'ibanez', 'foog',
    'grille', 'grille2', 'fourx12', 'cabmars', 'cabbend', 'cabbox',
    'brushed', 'matte', 'carbon', 'wood', 'chrome', 'vintage', 'flat',
  ];
  const _DEF_TEMPLATE = { amp: 'ampstd', cab: 'grille', pedal: 'box', rack: 'panel' };

  // Styles offered for a gear type — the type's primary first, then the rest.
  function templates(type) {
    type = _normType(type);
    const prim = _DEF_TEMPLATE[type];
    const keys = _TEMPLATE_ORDER.filter(k => {
      const t = _TEMPLATES[k];
      return t && (!type || !t.types || t.types.indexOf(type) >= 0);
    });
    if (prim && keys.indexOf(prim) > 0) { keys.splice(keys.indexOf(prim), 1); keys.unshift(prim); }
    return keys.map(k => ({ key: k, label: _TEMPLATES[k].label }));
  }

  function _bodyDrawFor(L) {
    const t = L.template && _TEMPLATES[L.template];
    if (t && (!t.types || t.types.indexOf(L.type) >= 0)) return t.draw;
    // Legacy 'default' / unknown / wrong-type → the type's primary style.
    const prim = _TEMPLATES[_DEF_TEMPLATE[L.type]];
    if (prim) return prim.draw;
    return L.type === 'amp' ? faceAmp : L.type === 'cab' ? faceCab : L.type === 'rack' ? faceRack : facePedal;
  }

  function _drawLabels(d, L) {
    const { W, H } = d;
    (L.controls || []).forEach(ct => {
      if (!ct.label) return;
      const lx = ct.cx * W;
      const below = ct.kind === 'knob' ? (ct.r * W * 1.12 + 8)
        : ct.kind === 'slider' ? 15
          : (ct.hs * W + 11);
      textC(d, lx, ct.cy * H + below, F.barlow, 11, rgb(208, 210, 216), ct.label.toUpperCase());
    });
  }

  // Turn a layout into a drawable/attachable P-style spec.
  function specFromLayout(layout) {
    const L = normalizeLayout(layout);
    const knobs = [], sliders = [], switches = [];
    L.controls.forEach(ct => {
      if (ct.kind === 'slider') {
        const half = Math.max(0.05, ct.r * 1.6);
        sliders.push({ id: ct.id, x0: ct.cx - half, x1: ct.cx + half, y: ct.cy, col: ct.color });
      } else if (ct.kind === 'switch') {
        switches.push({ id: ct.id, cx: ct.cx, cy: ct.cy, hs: ct.hs || 0.032, col: ct.color });
      } else {
        const k = { id: ct.id, cx: ct.cx, cy: ct.cy, r: ct.r || 0.06, style: 'pointer', cap: ct.color };
        if (ct.select > 2) k.select = ct.select;   // stepped "mode" knob (snaps to positions)
        knobs.push(k);
      }
    });
    const body = _bodyDrawFor(L);
    return {
      w: L.w, h: L.h, ptr: rgb(228, 226, 214), tick: rgb(120, 122, 128),
      knobs, sliders, switches, _layout: L,
      draw(d) { body(d, L); _drawLabels(d, L); _drawTicks(d, L); },
    };
  }

  // Draw the discrete value labels around a stepped/mode knob (e.g. a Pultec
  // band selector's 200 / 300 / 500 / 700 / 1000). `ticks` is the ordered list
  // of value strings captured from the plugin; positions fan across the knob's
  // 270° sweep, matching where the pointer lands at each step.
  function _drawTicks(d, L) {
    const { W, H } = d;
    (L.controls || []).forEach(ct => {
      if (ct.kind !== 'knob' || !Array.isArray(ct.ticks) || ct.ticks.length < 2) return;
      const cx = ct.cx * W, cy = ct.cy * H, R = (ct.r || 0.06) * W;
      const n = ct.ticks.length;
      for (let i = 0; i < n; i++) {
        const a = (135 + (i / (n - 1)) * 270) * Math.PI / 180;
        const lr = R * 1.62;
        textC(d, cx + Math.cos(a) * lr, cy + Math.sin(a) * lr, F.barlow, 8.5,
              rgb(198, 200, 208), String(ct.ticks[i]));
      }
    });
  }

  // attach()/dataURL() equivalents that take a spec OBJECT (custom gear) rather
  // than a registry stem.
  function attachSpec(canvas, spec, opts) { return attach(canvas, spec, opts || {}); }
  function dataURLSpec(spec, values) {
    if (!spec) return null;
    const cv = document.createElement('canvas'); cv.style.width = '220px';
    Object.defineProperty(cv, 'clientWidth', { value: 220, configurable: true });
    // Guard the draw so a single bad control/face never voids the whole photo —
    // return whatever rendered instead of null (which showed "no photo").
    try { drawSpec(cv, spec, values || {}); } catch (e) { try { console.warn('[rig_builder] face draw failed', e); } catch (_) {} }
    try { return cv.toDataURL('image/png'); } catch (_) { return null; }
  }
