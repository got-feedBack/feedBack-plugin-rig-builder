...
class RumbleverbPlugin : public Plugin {
    ...
    void applyAll(){ bool dty=fP[kChannel]>=0.5f; float g=dty?fP[kGain]:fP[kCleanVolume];core.configure(250e3,470e3,25e3,33e3,250e-12,22e-9,22e-9, dty?0.45f:0.25f, dty?8.0f:2.5f,13.0f,3000.0f,4.0f);core.setGain(g);core.setBass(0.5f);core.setMiddle(0.5f);core.setTreble(0.5f);core.setPresence(0.5f);core.setVolume(0.7f); }
public:
    RumbleverbPlugin() : Plugin(kParamCount,0,0){ for(int i=0;i<kParamCount;++i)fP[i]=kRumbleverbDef[i]; core.setSampleRate(kOS*(float)getSampleRate()); applyAll(); }
protected:
