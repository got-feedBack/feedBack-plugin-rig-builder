...
static constexpr float kGa79Lvl    = 0.408f;
...
protected:
    ...
    void run(const float** in, float** out, uint32_t frames) override {
        ...
        for (uint32_t i = 0; i < frames; ++i) {
            ...
            for (int k = 0; k < kOS; ++k) { ubL[k] = softClip(kGa79Makeup * L.process(ubL[k])); ubR[k] = softClip(kGa79Makeup * R.process(ubR[k])); }
            oL[i] = rbAmpLvl(kGa79Lvl * osL.downsample(ubL) * 0.98f); oR[i] = rbAmpLvl(kGa79Lvl * osR.downsample(ubR) * 0.98f);
        }
    }
