...
// Early-70s Hiwatt DR103, reconstructed from the local Mark Huss schematics:
// V1 ECC83 Normal/Brilliant -> A470k volumes + 470k mixers -> V2 ECC83
// -> passive Hiwatt TMB -> A220k master -> V3 ECC83 gain/bias follower
// -> fixed-bias ECC81 LTP -> 4x EL34 -> Partridge-style output transformer.
...
struct HiwattToneNetwork {
    ...
    void set(float sr,float bass,float treble,float middle,float master) {
        ...
        stampG(a,T,W,conductance((1.0-tp)*220000.0));
        stampG(a,W,B,conductance(tp*220000.0));
        ...
        stampG(a,D,-1,conductance(1.0+bp*470000.0));
        stampG(a,W,O,conductance(22000.0));

        const double mp=std::fmax(0.00001,std::fmin(0.99999,(double)rbtube::PotTaper::audio(master,1.70f)));
        stampG(a,O,M,conductance((1.0-mp)*220000.0));
        stampG(a,M,-1,conductance(mp*220000.0));
...
// The DR103 does not self-bias V4 like the reusable Fender/Marshall LTP. V3B
...
struct Dr103Core {
    ...
    void recalc(){
        ...
        normalCouple.set(sr,1.0f/(2.0f*kPi*470000.0f*22.0e-9f));
        ...
        const float hot=std::pow(std::fmax(brightPot,normalPot),2.0f);
        pi.set(sr,1.25f+1.10f*hot,1.0f);
        ...
        // BYX94 bridge, 2x220u series reservoir, 470R screen and 1k HT3 drop.
        supply.set(sr,16.0f,110.0f,470.0f,100.0f,1000.0f,220.0f,
                   .065f,.045f,.025f,.11f);
        power.set(sr,4.00f+4.00f*hot,-38.0f,.055f,32.0f,19000.0f);
        power.out=.0048f;
        power.biasShift=.75f;

        // Partridge TG/TH7549 primary/leakage response under the four-EL34
        // load. These broad corrections are inside the feedback loop and were
        // calibrated from the direct (cab-free) DR103 renders, not a speaker IR.
        otLowBody.peaking(sr,125.0f,.53f,8.5f);
        ...
        static const float normalDb[11]={
            -5.76f,-6.28f,-6.69f,-7.04f,-7.32f,-7.56f,-7.74f,-7.90f,-8.02f,-8.12f,-8.21f
        };
        static const float brightDb[11]={
            -6.28f,-6.69f,-7.03f,-7.30f,-7.53f,-7.73f,-7.90f,-8.05f,-8.18f,-8.29f,-8.39f
        };
        static const float bothDb[11]={
            -5.91f,-6.40f,-6.79f,-7.11f,-7.39f,-7.63f,-7.83f,-7.99f,-8.13f,-8.25f,-8.34f
        };
        const bool normalOnlyOut=pInput<.25f,brightOnlyOut=pInput>.75f;
        const float active=normalOnlyOut?pNormal:(brightOnlyOut?pBrilliant:std::fmax(pNormal,pBrilliant));
