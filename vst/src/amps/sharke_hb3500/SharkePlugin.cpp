 ...
 *
 * Hybrid dual-preamp bass head (Samson/Hartke main board 4005182800). All DSP
 ...
 * op-amp path (blend), built-in compressor, 10-band graphic EQ, variable HP/LP
 * tone filters, and a high-headroom SS power amp. See that header for the
 * topology and the EQ-band / power-headroom configuration.
 *
 * The HA3500 is a MONO amp (one 350 W power section) → single core, dual-mono
 * output (centered/balanced, half the CPU). The nonlinear chain (Newton 12AX7 +
 * op-amp rail clip + power-amp flat-top) runs at 2× oversampling (oversampler.hpp)
 * so the hard clipping stays alias-free.
 */
...
// Family loudness makeup (matches the "sharke_hb3500" calibrate_amp_core spec:
...
class SharkePlugin : public Plugin {
    sharke::SharkeCore core;                    // one mono core (HA3500 is mono)
...
public:
    SharkePlugin() : Plugin(kParamCount, 0, 0) {
        ...
        core.setEqFreqs(kEqFreqs, kNumEq);      // HA3500 10-band: 30..16k
        core.setPowerHeadroom(2.0f, 0.04f);     // 350 W → generous headroom
        core.reset(); recalc();
    }
protected:
    const char* getLabel()       const override { return "SharkeHB3500"; }
    const char* getDescription() const override { return "Hartke HA3500 bass head — circuit-real hybrid model"; }
