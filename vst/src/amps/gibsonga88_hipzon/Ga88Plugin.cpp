...
protected:
    ...
    void run(const float** in, float** out, uint32_t frames) override {
        ...
        for (uint32_t i = 0; i < frames; ++i) {
            ...
            for (int k = 0; k < kOS; ++k) { ubL[k] = softClip(kGa88Makeup * L.process(ubL[k])); ubR[k] = softClip(kGa88Makeup * R.process(ubR[k])); }
            oL[i] = rbAmpLvl(kGa88Lvl * osL.downsample(ubL) * 0.98f); oR[i] = rbAmpLvl(kGa88Lvl * osR.downsample(ubR) * 0.98f);
        }
    }
