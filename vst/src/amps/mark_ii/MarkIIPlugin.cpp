...
class MarkIIPlugin : public Plugin {
    ...
    void applyAll(){ bool ld=fP[kChannel]>=0.5f; float g=ld?fP[kLeadDrive]:fP[kVolume1]; float v=ld?fP[kLeadMaster]:fP[kMaster1];core.configure(250e3,250e3,25e3,100e3,500e-12,22e-9,22e-9, ld?0.45f:0.25f, ld?8.0f:3.0f,13.0f,3000.0f,4.0f);core.setGain(g);core.setBass(fP[kBass]);core.setMiddle(fP[kMiddle]);core.setTreble(fP[kTreble]);core.setPresence(0.5f);core.setVolume(v); }
public:
...
protected:
    ...
    const char* getDescription() const override { return "MarkII — circuit-real model"; }
