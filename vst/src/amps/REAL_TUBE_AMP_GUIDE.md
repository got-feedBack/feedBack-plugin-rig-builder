...
tablas y nuestro código**. El piloto terminado es **BOX DC30** (`en30/`, Vox AC30 Top
Boost). Esta guía es para portar el resto de amps (~60, hoy solo `en30` usa el modelo real).

> **Estado (2026-06-17):** BOX DC30 = **100 % circuito-real** ✅ — preamp (3×12AX7 + cátodo
> físico) + tonestack (Vox, Yeh) + power amp (EL84 class-A push-pull, sin NFB). Es la
> plantilla completa para portar los otros ~60 amps.
...
## 5. Valores por etapa del AC30 (ejemplo trabajado, BOX DC30)
...
cambia el bias, el sag y si hay NFB. BOX DC30 vs Ruby: crest 18.5/13.8/11.2 (limpio→rompe→
...
BOX DC30 hoy: crest 16.8/13.5/11.4 vs Ruby 17.5/12.0/12.1 (≤1.5 dB), estable.
...
| 6L6CG | 8.7   | 1.35 | 1460  | 4500  | 48.0  | 12.0 | 450     | 5.0k  | −21..21 |
...
5. **Calibrar** (§8): `stab.cpp` primero (estabilidad), luego `gain.cpp` (crest por nivel) y
   `wn.cpp` (EQ). Ajustar rangos, no inventar curvas.
...
- **BOX DC30 (`en30`)** — ✅ **100 % circuito-real** (preamp + tonestack + power amp). Plantilla de referencia.
...
- **Bassman (`tw40`, 5F6-A)** — ✅ 2×12AY7 (Bright/Normal) + 12AX7 + tonestack FMV (ya era real) + power (6L6 PP). Crest matchea `bassman_vol_4/12` (15.3/8.5 vs 15.3/9.1). PENDIENTE probar en vivo.
...
- **Framework `tube_stage.hpp`:** templates `TubeStageT`/`PowerAmpPPT` + traits. Tubos con tabla generada: 12AX7, 12AY7, EL84, 6V6, 6L6/5881, EL34. EQ circuito-real: `ToneStackYeh` (TMB de 3 bandas, R/C de cualquier amp) + `TweedTone` (control 1-perilla 5E3). Distorsión solid-state/diodo: `DiodeClipper` (Shockley, Newton — JC90/JC120/AOR50). **Los 11 amps tienen el EQ circuito-real** (BOX DC30/Bassman/Plexi/JCM800/DualRect/DSL100/MarkII/MarkIII/AOR50 = FMV/TMB; Deluxe = TweedTone; Super-Sonic = ToneStackYeh ×2). Agregar un tubo = constantes Koren en `gx_tube.py` + traits.
...
- **Todos los demás (~47)** — todavía caja-negra (`tanh`). Patrón de port: swap `triode/asymTube`→`TubeStage`, `power tanh`→`PowerAmpPPT`, agregar oversampling 2× al Plugin, calibrar crest vs render de referencia (o por carácter si no hay render). Próximos por uso: ENGL Powerball (`en50`, 146). El DSL15 (`dsl15_marsten`) reusa el DSL100 (EL84).
