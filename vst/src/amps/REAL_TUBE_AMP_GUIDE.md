...
- **Hiwatt DR103 (`dr103_lovolt`, = RS Amp_HG100)** — ✅ rework completo 2026-07-22 desde `hwpre1`, `hwpwr100w`, `hwpsu1` y los dibujos corregidos de Mark Huss. No comparte el core genérico del DR504: V1 ECC83 Normal/Brilliant con placas de 220k y red de cátodo compartida `1k5 + 100u`; acoples 22n/1n y A470k por canal; dos mitades V2 activas con placas mezcladas en la carga común de 100k, cátodos de 1k y red de 47n; red Hiwatt MNA de cinco capacitores (`1n`, `220p`, `1n`, `47n`, `47n`) con slope 100k y potes L220k/L100k/A470k; master A220k; V3A 100k/1k5 sin bypass, acople 47n y seguidor de bias; PI ECC81 **fixed-bias** propio (82k/91k y tail cargada), Presence dentro del NFB, B+ multinodo de diodos y 4×EL34 con respuesta OT Partridge. Cab Sim queda en `0`: el plugin es amp-only. Nueve referencias directas Brit DI (`Normal`, `Brilliant`, jumpeado × min/half/max) se comparan con `compare_nonlinear_reference.py`, incluyendo el inicio suave y terciles de nivel. Los 33 puntos finales quedan sin dropouts/clipping y dentro de 0.01 dB por compensación estrictamente post-circuito. Fuentes: esquema local `hwpre1.gif`, <https://hiwatt.org/tech2.html>, <https://ampbooks.com/mobile/classic-circuits/hiwatt-phase-inverter/>.
- **Hiwatt DR504 (`dr504_lovolt`, = RS Amp_HG500)** — ✅ rework completo 2026-07-22 desde `hwpre1.gif`, `hwpwr50w.gif`, `hwpsu2.gif` y `DR504_Complete.pdf`. El `hwpre1.gif` local es byte a byte idéntico al del DR103, por lo que conserva la misma ruta física: V1 ECC83 Normal/Brilliant con cátodo común `1k5 + 100u`, acoples 22n/1n, volúmenes A500k, dos mitades activas V2 mezcladas en la placa común, red Hiwatt MNA, V3A sin bypass, seguidor de bias y PI ECC81 fixed-bias. Sus diferencias no se clonaron del DR103: potes A500k/L250k/L100k y master L250k del layout; fuente BYX94 de 460V; bias `-36V`; 2×EL34 con pantallas de 1k; y OT Partridge TG6556/TH7551 de 50W. Esto entrega breakup y compresión de pantalla algo anteriores al DR103 sin convertirlo en Plexi. No existe referencia directa del DR504: se validó como miembro de familia contra el DR103 final usando el mismo Brit DI, no se declara *matched*. En el inicio suave a gain medio conserva el cuerpo (`-0.08 dB`) y presenta `+0.50 dB` de residuo no lineal quiet con `-0.21 dB` de crest, coherente con la menor reserva de potencia. Los 33 puntos finales (Normal-only, Brilliant-only y el recorrido Both usado por canciones con Normal `0.4`) quedan a `-19.90 dBFS`, sin clipping ni dropouts. Cab Sim `0`: amp-only.
