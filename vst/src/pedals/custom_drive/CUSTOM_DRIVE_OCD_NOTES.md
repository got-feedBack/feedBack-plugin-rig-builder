...
2N7000 MOSFET / 1N34A germanium clipping network between them, followed by a
...
MOSFET clipping network, so the DSP keeps the 1N34A branch.
...
- Clipper: two 2N7000 MOSFETs plus one 1N34A germanium diode produce
...
- Whether D1 should be jumper-only for a strict version III build. Current DSP
  follows `custom drive.png` and keeps the 1N34A branch.
...
- The main clipping block uses a nonlinear 2N7000/1N34A network solver instead
...
- Validation done in this pass: core syntax check and temp-dir VST3 build.
