 ...
 * signature op-amp grind. AUTO mode drives the sweep from an envelope follower
 * so it plays as a touch-wah without an expression pedal. RBAutoMakeup keeps
 * the engaged level matched to the dry input (character changes, loudness
 * doesn't). Per-channel cores; mono-summed pedal feed is not needed (a wah is a
 * per-channel filter).
 *
...
public:
    WH10WahPlugin() : Plugin(kParamCount, 0, 0) {
        ...
        makeupL.setSampleRate(sr); makeupR.setSampleRate(sr);
...
protected:
    ...
    void initParameter(uint32_t i, Parameter& p) override {
        ...
        if (i==(uint32_t)kAuto || i==(uint32_t)kRange) p.hints |= kParameterIsBoolean;
    ...
    void setParameterValue(uint32_t i, float v) override {
        if (i<(uint32_t)kParamCount){ params[i]=clamp01(v); recalc(); makeupL.snap(); makeupR.snap(); }
    }
    void sampleRateChanged(double r) override {
        left.setSampleRate((float)r); right.setSampleRate((float)r);
        makeupL.setSampleRate((float)r); makeupR.setSampleRate((float)r);
    ...
    void run(const float** in, float** out, uint32_t frames) override {
        ...
        for (uint32_t i=0;i<frames;++i){
            oL[i]=makeupL.process(iL[i], left.process(iL[i]));
            oR[i]=makeupR.process(iR[i], right.process(iR[i]));
        }
    }
